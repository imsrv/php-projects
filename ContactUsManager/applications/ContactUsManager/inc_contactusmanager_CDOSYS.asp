...
<%
Dim contactus__value13
contactus__value13 = "%"
If (Request.Form("ContactID")  <> "") Then 
  contactus__value13 = Request.Form("ContactID") 
End If
%>
<%
set contactus = Server.CreateObject("ADODB.Recordset")
contactus.ActiveConnection = MM_contactusmanager_STRING
contactus.Source = "SELECT tblContactUs.*, tblContactUsCategory.CategoryName  FROM tblContactUs INNER JOIN tblContactUsCategory ON tblContactUs.CategoryID = tblContactUsCategory.CategoryID  WHERE tblContactUs.Activated = 'True' AND ContactID LIKE '" + Replace(contactus__value13, "'", "''") + "'"
...
for i=1 to request.form.count
	...
	objMailAdmin.TextBody = "Message Sent To: " & contactus.Fields.Item("FirstName").Value  & " " & contactus.Fields.Item("LastName").Value & vbCrLf _
	& vbCrLf _
	& mailBody 
	objMailAdmin.Send
...
Set adminConf = Nothing
	...
	Set clientFlds = clientConf.Fields
				clientConf.Fields.Item("http://schemas.microsoft.com/cdo/configuration/sendusing") = 2
	...
	objMailClient.TextBody = "Thank you " & Request.Form("Name") & ", " & vbCrLf _
	& vbCrLf _
...
%>
          ...
          <tr> 
            <td width="25%" class="tableside"><b><font size="2" face="Arial, Helvetica, sans-serif"> 
              <input type="submit" value="Send Message" class="cCool">
</font></b></td>
