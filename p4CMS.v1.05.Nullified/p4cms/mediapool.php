...
<input name="button" type="button" class="button" onClick="this.disabled=true;this.value='Bitte warten...';document.forms['upform'].submit();" value="Hochladen"></td>
 	...
 	if (($endg==".jpg" || $endg==".gif" || $endg==".bmp" || $endg==".png")) {
 		...
 		if (isset($_REQUEST['resize'])) {
			...
			if (function_exists("imagecreatetruecolor")){
 			...
 			if ($error!=0) {
 				?>
 				<script>alert('Fehler: <?=$error;?>. Das Bild wurde in voller GrцЯe hochgeladen. Stellen Sie sicher dass das zu verkleinernde Bild als .jpg oder .png hochgeladen wurde und das GD-Libraries auf Ihrem Server aktiviert sind.');</script>
 				<?
 			}
 		}
 	} else {
 		...
 		<script language="javascript">
 		<!--
 		alert('Falscher Dateityp! Erlaubt sind fьr Videos nur .avi,.wmv und .mpg!');
 		//-->
 		</script>
 		<? 				
 			} else {
 		...
 		<script language="javascript">
 		<!--
 		alert('Falscher Dateityp! Erlaubt sind fьr Bilder nur .gif, .jpg, .bmp und .png!');
 		//-->
 		</script>
 		<?
 			}
 ...
 if ($_REQUEST['action']=="delfile") {
	if (@unlink("media" . $_REQUEST['file'])) {
		...
		<script language="javascript">
		<!--
		alert('Die Datei wurde erfolgreich gelцscht.');
		//-->
		</script>
		<?
	} else {
		...
		<script language="javascript">
		<!--
		alert('Unbekannter Fehler beim Lцschen der Datei.');
		//-->
		</script>
		<?
	}
...
<body bgcolor="#ffffff" topmargin="0" leftmargin="0">
 	...
 	function delfile(src) {
 		if (window.confirm('Wollen Sie die Datei ' + src + ' wirklich lцschen?')) {
...
function NewFolder() {
	var dname = window.prompt('Bitte geben Sie einen Namen fьr das Verzeichnis ein:', 'Neues Verzeichnis');
	if (dname=='' || dname==null) {
