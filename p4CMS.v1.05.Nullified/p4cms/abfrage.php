...
<!--
 function DelVar(id, titel) {
 	if (window.confirm('Wollen Sie die Variable "' + titel + '" wirklich komplett lцschen?')) {
...
while ($row = $sql->FetchRow()) {
					?>
      <tr bgcolor="#FAFAFB">
        <td height="17">&nbsp;{USER:<? echo($row->id); ?>} (<a href="javascript:DelVar('<?=$row->id;?>','<?=$row->titel;?>');">lцschen</a>)</td>
        <td height="17" bgcolor="#FAFAFB">&nbsp;<? echo(stripslashes($row->titel)); ?></td>
