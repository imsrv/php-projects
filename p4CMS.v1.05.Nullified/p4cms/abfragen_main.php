<?
 ...
 if ($grp['m_abfragen']=="no") {
 	StyleSheet();
	$msg = "<center>Sie besitzen nicht die Rechte, diese Aktion auszufьhren.</center>";
	MsgBox($msg);
	exit;
 }
 ...
 if ($_REQUEST['action']=="del") {
	...
	$sql->Close();
	eLog("user", "Abfrage $_REQUEST[id] gelцscht von $_SESSION[u_user]");
 	?>
...
<!--
 function DelAbf(id, titel) {
 	if (window.confirm('Wollen Sie die Abfrage "' + titel + '" wirklich komplett lцschen? In Dokumenten, die diese Abfrage noch nutzen, kцnnen Fehler auftreten!')) {
