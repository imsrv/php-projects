...
/* --------------------------------------------------------------
  ...
  function check_stock($products_id) {
    ...
    $attribute_stock_query = xtc_db_query("SELECT attributes_stock FROM " . TABLE_PRODUCTS_ATTRIBUTES . " where products_id = '" . $products_id . "'");
  ...
  function xtc_get_all_get_params($exclude_array = '') {
    ...
    while (list($key, $value) = each($_GET)) {
      if (($key != session_name()) && ($key != 'error') && (!xtc_in_array($key, $exclude_array))) $get_url .= $key . '=' . $value . '&';
    }
  ...
  function xtc_draw_products_pull_down($name, $parameters = '', $exclude = '') {
    ...
    $select_string .= '>';
    $products_query = xtc_db_query("select p.products_id, pd.products_name, p.products_price from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = pd.products_id and pd.language_id = '" . $_SESSION['languages_id'] . "' order by products_name");
  ...
  function xtc_get_category_name($category_id, $language_id) {
