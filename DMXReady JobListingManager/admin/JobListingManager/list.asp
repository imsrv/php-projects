...
If (CStr(Request("MM_insert")) = "insert") Then
  ...
  MM_fieldsStr  = "CategoryID|value|ItemName|value|DateAdded|value|Activated|value|SendToEmailAddress|value|SendToInstructions|value"
  MM_columnsStr = "CategoryIDkey|none,none,NULL|ItemName|',none,''|DateAdded|',none,NULL|Activated|',none,''|SendToEmailAddress|',none,''|SendToInstructions|',none,''"
...
rsItems.Source = "SELECT tblItems_Category_1.CategoryValue AS ParentCategoryValue, tblItems_Category.*, tblItems.*  FROM (tblItems_Category RIGHT JOIN tblItems ON tblItems_Category.CategoryID = tblItems.CategoryIDkey) LEFT JOIN tblItems_Category AS tblItems_Category_1 ON tblItems_Category.ParentCategoryID = tblItems_Category_1.CategoryID  WHERE (((tblItems.ItemDesc) Like '%" + Replace(rsItems__MMColParam1, "'", "''") + "%') AND ((tblItems.ItemID) Like '" + Replace(rsItems__value1, "'", "''") + "') AND ((tblItems.CategoryIDkey) Like '" + Replace(rsItems__value3, "'", "''") + "')) OR (((tblItems.ItemID) Like '" + Replace(rsItems__value1, "'", "''") + "') AND ((tblItems.CategoryIDkey) Like '" + Replace(rsItems__value3, "'", "''") + "') AND ((tblItems.ItemName) Like '%" + Replace(rsItems__MMColParam1, "'", "''") + "%')) OR (((tblItems.ItemID) Like '" + Replace(rsItems__value1, "'", "''") + "') AND ((tblItems.CategoryIDkey) Like '" + Replace(rsItems__value3, "'", "''") + "') AND ((tblItems.ItemMemo) Like '%" + Replace(rsItems__MMColParam1, "'", "''") + "%'))  ORDER BY tblItems.DateAdded, tblItems_Category_1.CategoryID , tblItems.ItemID"
  ...
  <tr> 
    <td valign="top"> 
	  ...
	  <table width="100%" align="center" class="tableborder">
          ...
          <tr valign="middle">
            <td width="0" class="tableheader"> Select Department  </td>
...
%>
          ...
          <tr valign="middle">
            <td class="tableheader">Enter Job Title </td>
            <td colspan="2" class="tableheader">
            ...
            <input name="SendToEmailAddress" type="hidden" id="SendToEmailAddress" value="@">
			<input name="SendToInstructions" type="hidden" id="SendToInstructions" value="Please provide a cover letter with your resume">
</td>
          </tr>
        </table>
...
<table width="100%" height="32" border="0" cellpadding="2" cellspacing="0" class="tableborder">
    <tr class="tableheader">
      <td width="0">&nbsp; </td>
      <td width="0">Division <a href="javascript:;" onClick="MM_openBrWindow('CategoryManager/admin.asp','Category','scrollbars=yes,width=600,height=400')">[add/edit]</a></td>
      <td width="0"> Department <a href="javascript:;" onClick="MM_openBrWindow('CategoryManager/admin.asp','Category','scrollbars=yes,width=600,height=400')">[add/edit]</a></td>
      <td width="0">Date Added </td>
      <td width="0">Job ID</td>
      <td width="0">Job Title </td>
      <td width="0">Send Application To </td>
...
%>">
      ...
      <td width="0"><%=(rsItems.Fields.Item("DateAdded").Value)%></td>
      ...
      <td width="0" height="13"><input name="<%=(iCount & ".SendToEmailAddress")%>" type="text" value="<%=(rsItems.Fields.Item("SendToEmailAddress").Value)%>" size="40">
      </td>
      ...
      <td width="0" height="13" align="center"> <a href="update.asp?ItemID=<%=(rsItems.Fields.Item("ItemID").Value)%>&action=detail"> Edit Details</a> </td>
      ...
      <td width="0" height="13">
        <div align="center"><a href="/applications/JobListingManager/inc_joblistingmanager.asp?ItemID=<%=(rsItems.Fields.Item("ItemID").Value)%>&CategoryID=<%=(rsItems.Fields.Item("CategoryID").Value)%>" target="_blank">Live
            Preview</a> </div>
      </td>
    </tr>
    <% 
