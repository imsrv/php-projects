...
sub register_form {
	...
	@rhlist = split (/\./,$remote_host);
	$domain = "$rhlist[$#rhlist-1].$rhlist[$#rhlist]";
...
sub register {
	...
	@rhlist = split (/\./,$remote_host);
	$domain = "$rhlist[$#rhlist-1].$rhlist[$#rhlist]";
	$email .=$domain if $force_domain eq 'yes' and ($email);
	...
	if (($email =~ /.+?\@.+?\..+?/) and ($name)) {
		...
		print USERFILE "$name\t$password\t$email\t$time\t$status\n";
...
sub gateway {
	...
	if($password) {
		...
		for $info (@user_info) {
			($nickname, $pass, $email, $time, $status) = split (/\t/, $info);
			next if not $pass;

			 if($nickname eq $name and $password eq $pass) {
				$is_user = 'yes';
				$time = time;
				...
				$info = join ("\t", ($nickname, $pass, $email, $time, $status));
				next;
			}

			$is_nick = 'yes' if $nickname eq $name and $password ne $pass;

			if(($free eq 'yes') or ($status eq '')) {
				$time = 'delete' if time > $time + 60*60*24*$timeout;
				$info = join ("\t", ($nickname, $pass, $email, $time, $status));
			}else {
				if(($status =~ /^\d+$/) and ($status - 60*60*24*3 < time)) {
					...
					if(-d "$datapath/$user_path") {
						...
						closedir(DIR);
						for $item(@raw) {
							if(-T "$datapath/$user_path/$item") {
								unlink("$datapath/$user_path/$item");
							}
							if(-B "$datapath/$user_path/$item") {
								unlink("$datapath/$user_path/$item");
							}
						}
						for (1..10) {
							last if rmdir("$datapath/$user_path");
						}
					}
					last if $is_nick or $is_user;
				}
		...
		for $line(@user_info) {
			next if $line eq '';
			print USERFILE "$line\n" if $line !~ /\tdelete\b/i and $line !~ /\tbanned\b/i;
			print XLOG "$line\n" if $line =~ /\tdelete\b/i and $free ne 'yes';
			print XLOG "$line\n" if $line =~ /\tbanned\b/i;
		}
		close (XLOG);
		close(USERFILE);
...
sub match {
	...
	#############################################
	# Load next 250 objects profiles

	if(exists($FORM{'index'})) {
		$index = $FORM{'index'}
	}else {
		$index = 0;
	}
	...
	chomp(@password_list);
	$end_index = $index + 249;
	$end_index = $#password_list if $end_index > $#password_list;
	%score_list = ();
	for ($i=$index;$i<=$end_index;$i++) {
		next unless($password_list[$i]);
		next if $password_list[$i] eq $FORM{'password'};
		$user_path = &get_user_path($password_list[$i]);
		next unless (-e "$datapath/$user_path/profile.txt");
		unless(open(PROFILE,"$datapath/$user_path/profile.txt")) {
			&system_error("Can not open $datapath/$user_path/profile.txt.\n");
			exit;
		}
		@data = <PROFILE>;
		close(PROFILE);
		...
		&get_file_names("$FORM{'password'}", "$users{$object_name}");
	...
	if($end_index < $#password_list - 1) {
		print "Content-type: text/html\n\n";
		print "<head><title>Matching</title>\n";
		$index++;$end_index++;
		print "<META HTTP-EQUIV=Refresh CONTENT=\"2;URL=$ENV{'SCRIPT_NAME'}?mode=match&nickname=$esc_nick&ID=$FORM{'ID'}&index=$end_index\">\n";
		print "$header\n";
		print "<h1><center>Matching...</center></h1><hr>\n";
		print "...users $index through $end_index.<br>\n";
		print "Hang on...\n";
		print "$footer\n";
		exit;
	}else {
...
sub change_password {
	...
	for ($i=0;$i<=$#userdata;$i++) {
		($nickname, $password, $email, $stuff) = split(/\t/, $userdata[$i]);
		if ($password eq $FORM{'password'}) {
...
sub main_menu {
	...
	print "<a class=light href=$search_script?nickname=$esc_nick&ID=$FORM{'ID'} target=new>[Quick Search]</a>\n";
...
sub list_matches {
	...
	for $name (sort{$match_list{$b} <=> $match_list{$a}} keys (%match_list)) {
		...
		print "><td>Nickname: <b>$name</b> - Rating: <b>$score_text</b></td><td><a href=$ENV{'SCRIPT_NAME'}?nickname=$esc_nick&ID=$FORM{'ID'}&mode=viewmatch&match=$esc_match_name>[View $name\'s Profile]</a></td>\n";
...
sub make_ID {
	...
	open(IDS,"+>>$logpath/id.txt") || &return_page('System Error', "Can't write.(2373)\n");
	 ...
	 $time = time;
	 push(@lines, "$this_ID\t$password\t$ENV{'REMOTE_ADDR'}\t$time\n");
...
sub get_pass {
	...
	foreach $line (@lines) {
		($this_ID, $this_password, $this_addr) = split(/\t/, $line);
		return ($this_password) if $this_ID eq $ID and $this_addr eq $ENV{'REMOTE_ADDR'};
	}
