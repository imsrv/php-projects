...
If (CStr(UploadFormRequest("MM_update")) <> "" And CStr(UploadFormRequest("MM_recordId")) <> "") Then
  ...
  MM_columnsStr = "ImageFileB2|',none,''"
...
%>
<%
Dim imageB2__MMColParam
imageB2__MMColParam = "0"
If (Request.QueryString("mid2")   <> "") Then 
  imageB2__MMColParam = Request.QueryString("mid2")  
End If
%>
<%
set imageB2 = Server.CreateObject("ADODB.Recordset")
imageB2.ActiveConnection = MM_sitechassismanager_STRING
imageB2.Source = "SELECT tblSitePlanNavMenu.*, tblSitePlanNavMenu2.*, tblSitePlanNavMenu3.*  FROM (tblSitePlanNavMenu LEFT JOIN tblSitePlanNavMenu2 ON tblSitePlanNavMenu.mid = tblSitePlanNavMenu2.midkey) LEFT JOIN tblSitePlanNavMenu3 ON tblSitePlanNavMenu2.mid2 = tblSitePlanNavMenu3.mid2key  WHERE mid2 = " + Replace(imageB2__MMColParam, "'", "''") + ""
imageB2.CursorType = 0
imageB2.CursorLocation = 2
imageB2.LockType = 3
imageB2.Open()
imageB2_numRows = 0
%>
...
<form ACTION="<%=MM_editAction%>" METHOD="post" name="form1" enctype="multipart/form-data" onSubmit="checkFileUpload(this,'GIF,JPG,JPEG,BMP,PNG');checkFileUpload(this,'GIF,JPG,JPEG,BMP,PNG',true,'','','','','','','');return document.MM_returnValue">
  <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableborder">
    <tr valign="baseline"> 
      ...
      <td width="96%"> Image will be added to the following page: <strong><%=(imageB2.Fields.Item("Menu").Value)%>
	  
        <% if imageB2.Fields.Item("Menu2").Value <> "" then %>
	   - <%=(imageB2.Fields.Item("Menu2").Value)%>
       <% end if%>
       <% if imageB2.Fields.Item("Menu3").Value <> "" then %>
	   - <%=(imageB2.Fields.Item("Menu3").Value)%>
       <% end if%> 
	   </strong></td>
    </tr>
    <tr valign="baseline"> 
  ...
  <input type="hidden" name="MM_recordId" value="<%= imageB2.Fields.Item("mid2").Value %>">
</form>
</body>
</html>
<%
