...
imageB__MMColParam = "0"
If (Request.QueryString("mid")    <> "") Then 
  imageB__MMColParam = Request.QueryString("mid")   
End If
%>
<%
set imageB = Server.CreateObject("ADODB.Recordset")
imageB.ActiveConnection = MM_sitechassismanager_STRING
imageB.Source = "SELECT tblSitePlanNavMenu.*, tblSitePlanNavMenu2.*, tblSitePlanNavMenu3.*  FROM (tblSitePlanNavMenu LEFT JOIN tblSitePlanNavMenu2 ON tblSitePlanNavMenu.mid = tblSitePlanNavMenu2.midkey) LEFT JOIN tblSitePlanNavMenu3 ON tblSitePlanNavMenu2.mid2 = tblSitePlanNavMenu3.mid2key  WHERE mid = " + Replace(imageB__MMColParam, "'", "''") + ""
imageB.CursorType = 0
...
<form ACTION="<%=MM_editAction%>" METHOD="post" name="form1" enctype="multipart/form-data" onSubmit="checkFileUpload(this,'GIF,JPG,JPEG,BMP,PNG');checkFileUpload(this,'GIF,JPG,JPEG,BMP,PNG',true,'','','','','','','');return document.MM_returnValue">
  <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableborder">
    <tr valign="baseline"> 
      ...
      <td width="96%"> Image will be added to the following page: <strong><%=(imageB.Fields.Item("Menu").Value)%>
	  
        <% if imageB.Fields.Item("Menu2").Value <> "" then %>
	   - <%=(imageB.Fields.Item("Menu2").Value)%> 
	   <% end if%>
	   <% if imageB.Fields.Item("Menu3").Value <> "" then %>
	   - <%=(imageB.Fields.Item("Menu3").Value)%>
	   <% end if%> 
	   </strong></td>
    </tr>
    <tr valign="baseline"> 
  ...
  <input type="hidden" name="MM_recordId" value="<%= imageB.Fields.Item("mid").Value %>">
</form>
</body>
</html>
<%
