...
?>
			...
			<B><DIV width=100% class=highlight>Please confirm the following before depositing funds:<br><BR>
				Your transfer amount: <?=prsumm($amount)?><BR>
				Processing Fee: -<?=prsumm($fee)?><BR>
				Total Account Debit: <?=prsumm($amount + $fee)?><BR>
			</DIV>
			<CENTER>
...
?>
<?
	function ChargeAuthorizeNet($user,$row,$amount){
		...
		array_push($qarray, "x_Email=" . urlencode($email));
		...
		switch ($rarray[0]) {
			case 1:
				$r = mysql_fetch_object(mysql_query("SELECT * FROM radlance_users WHERE suid='".$suid."'"));
				if ($r){
					transact(17,$r->id,($amount-$fees),'Deposit','',$fees,1,'',addslashes($orderno));
					// Notify admin
