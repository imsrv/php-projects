...
<?
	...
	$ulist_page = 3;
	if(!$startp){$startp = 0;}
	$qr2 = mysql_query("SELECT paidto,paidby,amount,epay_transactions.comment AS comment,trdate,id,fees FROM epay_transactions WHERE id='$did' AND pending=0 ORDER BY trdate DESC") or die( mysql_error() );
	$numrows = mysql_num_rows($qr2);
	$r = mysql_query("SELECT paidto,paidby,amount,epay_transactions.comment AS comment,trdate,id,fees FROM epay_transactions WHERE id='$did' AND pending=0 ORDER BY trdate DESC LIMIT $startp,$ulist_page") or die( mysql_error() );
	$i = 1;
