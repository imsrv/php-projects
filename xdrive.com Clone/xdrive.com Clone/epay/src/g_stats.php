	...
	function downline($user,$curLev,$date="" ){
		...
		if($qr1){
			while( $row = mysql_fetch_object($qr1) ){
				$myLev = $curLev + 1;
				if($myLev > 6)return;
				$aqr1 = mysql_query("SELECT COUNT(*) AS sales FROM epay_transactions WHERE paidto='$user' AND paidby='99' AND relatedproject='{$row->id}' AND trdate LIKE '$adate%'");
				if($aqr1){
					$arow = mysql_fetch_object($aqr1);
					$sales = $arow->sales;
				}
				$aqr1 = mysql_query("SELECT SUM(amount) AS earned FROM epay_transactions WHERE paidto='$user' AND paidby='99' AND relatedproject='{$row->id}' AND trdate LIKE '$adate%'");
				if($aqr1){
					$arow = mysql_fetch_object($aqr1);
					$earned = $arow->earned;
				}
