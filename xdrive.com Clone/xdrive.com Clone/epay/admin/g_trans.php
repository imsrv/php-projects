...
if (!$_fpr_add)
{
  $_fpr_data = mysql_fetch_array(mysql_query(
    "SELECT * FROM epay_transactions WHERE id='{$_GET['id']}'"
  ), MYSQL_ASSOC);
  $_fpr_orig = $_fpr_data;
}
...
if ($_POST['0b'] && !$_fpr_err)
{
  ...
  {
    $i = mysql_query("UPDATE epay_transactions SET amount='{$j['amount']}',comment='{$j['comment']}',pending='{$j['pending']}',orderno='{$j['orderno']}',addinfo='{$j['addinfo']}',fees='{$j['fees']}' WHERE id='{$_GET['id']}'");
  }
  if (!$i) $_fpr_err = 'query';
}
else
