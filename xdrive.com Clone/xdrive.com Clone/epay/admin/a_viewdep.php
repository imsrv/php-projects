...
<?
$qr1 = mysql_query("SELECT COUNT(*) FROM epay_transactions WHERE (paidby>10 AND paidby<100) AND pending=$pending");
list($total) = mysql_fetch_row($qr1);

$limit = (int)$_GET['from'].",".$max_elements;
$qr1 = mysql_query("SELECT * FROM epay_transactions WHERE (paidby>10 AND paidby<100) AND pending=$pending ORDER BY trdate DESC LIMIT $limit");
...
<TR><TD>
  ...
  <?=( $pending ? "<td><a href=# onClick=\"form1.confirm.value=$a->id; form1.submit(); return false;\">confirm</a> <a href=right.php?a=viewdep&del=$a->id&$id>delete</a>" : "" )?>
<?
  if ($a->addinfo)
