...
$PAYER_AMOUNT = $_REQUEST['PAYER_AMOUNT'];
$amount = $PAYER_AMOUNT;
$fees = myround($amount * $dep_eg_percent / 100, 2) + $dep_eg_fee;
...
if( ($HASH == md5("abdo".$CHECKSUM.$suid)) && ($PAYMENT_BATCH_NUM != 0) && (!empty($PAYER_ACCOUNT)) ){
	...
	if ($r ){
		transact(15,$r->id,($amount-$fees),'Deposit','',$fees,1,'',addslashes($orderno));
		// Notify admin
		$message = $GLOBALS[$r->type]." $r->username has just deposited {$currency}$amount via E-Gold!";
		if ($dep_notify){
