...
function is_local_file($file_name) {
  return !is_remote($file_name) && basename($file_name) != $file_name && preg_match("#\.[a-zA-Z0-9]{1,4}$#", $file_name) ? 1 : 0;
}
...
function check_local_media($local_media_file) {
  global $config;
  return !is_remote($local_media_file) && basename($local_media_file) != $local_media_file && preg_match("#\.[".$config['allowed_mediatypes_match']."]+$#i", $local_media_file) ? 1 : 0;
}
...
function check_local_thumb($local_thumb_file) {
  return !is_remote($local_thumb_file) && basename($local_thumb_file) != $local_thumb_file && preg_match("#\.[gif|jpg|jpeg|png]+$#i", $local_thumb_file) ? 1 : 0;
}
...
function get_categories($cat_id = 0) {
  ...
  foreach ($visible_cat_cache as $key => $category_id) {
    ...
    $site_template->register_vars(array(
      "cat_id" => $category_id,
      "cat_name" => format_text($cat_cache[$category_id]['cat_name'], 0),
