...
function delete_comments($comment_ids) {
  ...
  while ($comment_row = $site_db->fetch_array($comment_result)) {
    ...
    if ($del_comment) {
      ...
      echo "<b>".$lang['comment_delete_success'].":</b> ".$comment_row['comment_headline']." (".$lang['user'].": ".$comment_row['user_name'].")<br />\n";
    }
    else {
      $error_log[] = "<b>".$lang['comment_delete_error'].":</b> ".$comment_row['comment_headline']." (".$lang['user'].": ".$comment_row['user_name'].")";
    }
    echo "<br />\n";
  }
  return $error_log;
...
if ($action == "editcomment") {
  ...
  show_table_header($lang['nav_comments_edit'].": ".$comment['comment_headline'], 2);
...
if ($action == "findcomments") {
  ...
  if ($countcomments['comments'] > 0) {
    ...
    while ($comment_row = $site_db->fetch_array($result)) {
      ...
      $show_comment .= $comment_row['comment_text'];
      echo "<td>".$show_comment."</td>\n";
      $show_user_name = format_text($comment_row['user_name']);
      ...
      $show_image = "<a href=\"".$site_sess->url(ROOT_PATH."details.php?".URL_IMAGE_ID."=".$comment_row['image_id'])."\" target=\"_blank\">".$comment_row['image_name']."</a> (ID: ".$comment_row['image_id'].")";
