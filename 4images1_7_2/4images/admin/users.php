  ...
  while ($user_row = $site_db->fetch_array($user_result)) {
    ...
    if (!defined("USER_INTEGRATION") || (defined("USER_INTEGRATION") && USER_INTEGRATION == "NONE")) {
      ...
      if ($site_db->query($sql)) {
        echo "<b>".$lang['user_delete_success']."</b> ".$user_name." (ID: $user_id)<br />\n";
      }
      else {
        $error_log[] = "<b>".$lang['user_delete_error']."</b> ".$user_name." (ID: $user_id)<br />\n";
      }
    }
    else {
      echo "<b>".$lang['user_integration_delete_msg']."</b> ".$user_name." (ID: $user_id)<br />\n";
    }
    ...
    if ($delimages) {
      ...
      if ($site_db->query($sql)) {
        ...
        $error_log[] = $lang['images_delete_error'].": ".$user_name;
      }
    }
    else { //Update Images
      ...
      if ($site_db->query($sql)) {
        ...
        $error_log[] = $lang['user_images_update_error'].": ".$user_name;
      }
    }
...
if ($action == "updateuser") {
  ...
  if ($user_name != "" && strtolower($current['user_name']) != strtolower(stripslashes($user_name)) && $site_db->not_empty("SELECT ".get_user_table_field("", "user_name")." FROM ".USERS_TABLE." WHERE ".get_user_table_field("", "user_name")." = '".strtolower($user_name)."'")) {
    $msg .= sprintf("%s: <b>%s</b><br />", $lang['user_name_exists'], $user_name);
    $error['user_name'] = 1;
  }
...
if ($action == "edituser") {
  ...
  show_table_header($lang['edit'].": ".$user_row['user_name'], 2);
...
if ($action == "findusers") {
  ...
  if ($countusers['users'] > 0) {
    ...
    while ($user_row = $site_db->fetch_array($result)) {
      ...
      $show_user_name = format_text($user_row[$user_table_fields['user_name']]);
  ...
  for ($i = 1; $i <= $num_newusers; $i++) {
    ...
    if ($user_name != "") {
      ...
      if ($site_db->not_empty($sql)) {
        $msg .= sprintf("%s: <b>%s (%s %s)</b><br />", $lang['user_name_exists'], $user_name, $lang['user'], $i);
        $error['user_name_'.$i] = 1;
      }
    }
  ...
  if (empty($error)) {
    for ($i = 1; $i <= $num_newusers; $i++) {
      ...
      if ($result) {
        ...
        $site_db->query($sql);
        $log[] = $lang['user_add_success']." : <b>$user_name</b>";
      }
      else {
        $log[] = $lang['user_add_error'].": <b>$user_name</b>";
      }
