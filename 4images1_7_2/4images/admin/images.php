...
function delete_images($image_ids, $delfromserver = 1) {
  ...
  while ($image_row = $site_db->fetch_array($image_result)) {
    ...
    if ($site_db->query($sql)) {
      ...
      $error_log[] = "<b>".$lang['image_delete_success']."</b> ".$image_row['image_name']." (ID: ".$image_row['image_id'].")<br />";
    }
...
if ($action == "editimage") {
  ...
  show_table_header($lang['nav_images_edit'].": ".$image_row['image_name'], 2);
...
if ($action == "findimages") {
  ...
  if ($countimages['images'] > 0) {
    ...
    while ($image_row = $site_db->fetch_array($result)) {
      ...
      echo "<td><b><a href=\"".$site_sess->url(ROOT_PATH."details.php?".URL_IMAGE_ID."=".$image_row['image_id'])."\" target=_blank>".format_text($image_row['image_name'])."</a></b> (".$image_row['image_media_file'];

      if (!get_file_path($image_row['image_media_file'], "media", $image_row['cat_id'], 1, 0)) {
        echo " <b class=\"marktext\">!</b>";
      }
      echo ")</td>\n";
      echo "<td><a href=\"".$site_sess->url(ROOT_PATH."categories.php?".URL_CAT_ID."=".$image_row['cat_id'])."\" target=\"_blank\">".format_text($cat_cache[$image_row['cat_id']]['cat_name'])."</a></td>\n";
      $show_user_name = format_text($image_row[$user_table_fields['user_name']]);
