...
if ($action == "createthumbnails") {
  ...
  if (!empty($image_list)) {
    ...
    foreach ($image_list as $key => $val) {
      if ($val == 1) {
        echo "<p>".$lang['creating_thumbnail'].$image_cache[$key]['image_name']." (".$image_cache[$key]['image_media_file'].") ....&nbsp;&nbsp;\n";
        flush();
        @set_time_limit(90);
    ...
    while ($image_row = $site_db->fetch_array($result)) {
      if ((!file_exists(THUMB_PATH."/".$image_row['cat_id']."/".$image_row['image_thumb_file']) || $image_row['image_thumb_file'] == "") && file_exists(MEDIA_PATH."/".$image_row['cat_id']."/".$image_row['image_media_file'])) {
        ...
        if ($image_info[2] == 1 || $image_info[2] == 2 || $image_info[2] == 3) {
          ...
          $imgs .= "<td width=\"30%\"><b><a href=\"".$site_sess->url(ROOT_PATH."details.php?".URL_IMAGE_ID."=".$image_row['image_id'])."\" target=_blank>".$image_row['image_name']."</a></b></td>\n";
          $imgs .= "<td width=\"25%\">".$image_row['image_media_file']."</td>\n";
          $imgs .= "<td width=\"25%\">".$cat_cache[$image_row['cat_id']]['cat_name']."</td></tr>\n";
          $image_counter++;
        }
      }
