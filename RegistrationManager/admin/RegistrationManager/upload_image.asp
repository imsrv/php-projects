...
If (CStr(Request.QueryString("GP_upload")) <> "") Then
  ...
  pau_thePath = """../../applications/RegistrationManager/images"""
...
If (Request.QueryString("MemberID")          <> "") Then 
  images__value1 = Request.QueryString("MemberID")         
End If
%>
<%
set images = Server.CreateObject("ADODB.Recordset")
images.ActiveConnection = MM_registrationmanager_STRING
images.Source = "SELECT *  FROM tblMM_Members  WHERE MemberID = " + Replace(images__value1, "'", "''") + ""
images.CursorType = 0
...
<form ACTION="<%=MM_editAction%>" METHOD="post" enctype="multipart/form-data" name="imageA" id="imageA" onSubmit="checkFileUpload(this,'GIF,JPG,JPEG,BMP,PNG',true,'','','','','','','');return document.MM_returnValue">
  <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableborder">
    <tr valign="baseline"> 
      ...
      <td width="96%"> Upload Image: <strong><%=(images.Fields.Item("FirstName").Value)%>&nbsp; <%=(images.Fields.Item("LastName").Value)%></strong></td>
    </tr>
    <tr valign="baseline"> 
...
<input type="hidden" name="MM_recordId" value="<%= images.Fields.Item("MemberID").Value %>">
</form>
</body>
</html>
<%
