...
if ($type=="reply") {
	...
	// Remove PGP's sigs and normal sigs in pgp signed messages
	$sig = array_search("-----BEGIN PGP SIGNED MESSAGE-----",$body);
	if ($sig==1) {
		$body = array_slice($body,3);
		$sig = array_search("-----BEGIN PGP SIGNATURE-----",$body);
		$body = array_splice($body,0,$sig);
		$sig = array_search("- -- ",$body);
		if ($sig!=0) {
			$body = array_splice($body,0,$sig);
			}
	}	
	// Remove normal sigs in no pgp signed messages
	...
	if ($c==0){
		...
		$quot = preg_match_all("/^\s*(&gt;)(\s|(&gt;))*/",$body[$k],$gt,PREG_PATTERN_ORDER);
		$num_quot = substr_count($gt[0],"&gt;");
		switch ($num_quot) {
		case 0:
			$bodyzeile=$bodyzeile."> ".$body[$k]."\n";
			for ($i=$k+1; $i<count($body); $i++) {
				if (trim($body[$i])!="") {
				$quot_tmp = preg_match_all("/^\s*&gt;(\s|&gt;)*/",$body[$i],$gt_tmp);
    	    	$num_quot_tmp=substr_count($gt_tmp[0][0],"&gt;");
			    	if ($num_quot_tmp!=0) {
						$bodyzeile=$bodyzeile.">".$body[$i]."\n";
			    	} else {
					$quot_tmp2 = preg_match_all("/^\s*&gt;(\s|&gt;)*/",$body[$i - 1],$gt_tmp2);
					$quot_tmp3 = preg_match_all("/^\s*&gt;(\s|&gt;)*/",$body[$i],$gt_tmp3);
					$num_quot_tmp2=substr_count($gt_tmp2[0][0],"&gt;");
					$num_quot_tmp3=substr_count($gt_tmp3[0][0],"&gt;");
					if ($num_quot_tmp3!=$num_quot_tmp2) {
						$body[$i]=str_pad($body[$i],strlen($body[$i]) + 1," ",STR_PAD_LEFT);
						$body[$i]=str_pad($body[$i],strlen($body[$i]) + $num_quot_tmp2,">",STR_PAD_LEFT);
						$bodyzeile=$bodyzeile.">".$body[$i]."\n";
					} else {
						$bodyzeile=$bodyzeile."> ".$body[$i]."\n";
						}
