...
*************************************************************************************************/ if(!defined('CONFIG')) die;
 /**
  * Tournament Module
  * Double Elimation Class
 */

 require_once('./code/class/modules/elimination/bracket.inc.php');

 class module_elim_bracket_double extends elimination_bracket {
  function module_elim_bracket_double($id){
  ...
  function show(){global $tpl, $teams, $users, $tourny;
   ...
   foreach($this->matchs() as $match)
    for($t=0;$t < $this->get("teamspermatch");$t++){
     ...
     if($team->id > 0)
      $tpl->assign($match->get_team($t, "position"), $tpl->fetchfile("module_dblelim_setup_team.tpl", array(
  ...
  function write_setup(){global $tourny, $apanel, $tpl, $teams, $users;
   ...
   if($_GET["setteam"] > 0){
    if(isset($_GET["newteam"])){ //set team
     ...
     $tpl->splice("SETUP", "module_dblelim_setup_setteam.tpl");
   ...
   $tpl->parsefile("CONTENT", "module_dblelim_setup.tpl");
 ...
 function write_presetup_module_double_elim(){global $tourny, $apanel, $tpl;
  $tpl->splice("SETUP", "module_dblelim_presetup.tpl");
  ...
  $tpl->assign(array(
    ...
    "FIELD_NAME_VALUE" => (strlen($_POST["name"]) >= 5 && strlen($_POST["name"]) <= 125) ? $_POST["name"] : ($_POST["name"] == '' ? "Double Elimination Bracket" : substr($_POST["name"], 0, 125) ),
    "FIELD_TPM_NAME"   => "tpm",
