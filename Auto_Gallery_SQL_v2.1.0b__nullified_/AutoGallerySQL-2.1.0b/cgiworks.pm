...
use vars qw( 
             ...
             $O_EXCL $O_APPEND $O_TRUNC $O_NONBLOCK $DEL $O_REC_LONG $DS_CACHE $ERRORS
             $GAL_COUNT %TPL %FRM %QRY %MTHS %DAYS
           );
...
$ERRLOG  = 1;                          ##  Change this to a 1 (one) if you want to log error messages
...
sub diskspace {
  ...
  sysopen(FILE, "$DDIR/test.file", $O_WRONLY | $O_CREAT) || return;
  flock(FILE, $LOCK_EX);
  truncate(FILE, 0);
...
sub err {
  ...
  if( $ERRLOG && open(FILE, ">>$DDIR/error.log") )
  {
    flock(FILE, $LOCK_EX);
    print FILE "[ " . scalar(localtime()) . " ]  [ $file ]  [ $cause ]\n";
    flock(FILE, $LOCK_UN);
    close(FILE);
    chmod(0666, $file) if( -O $file );
  }
