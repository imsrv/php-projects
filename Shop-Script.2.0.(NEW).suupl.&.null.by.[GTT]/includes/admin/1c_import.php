 ...
 ****************************************************************************/
	...
	//ADMIN:: import from 1С

	//этот файл актуален только для русскоязыных пользователей
	//поэтому здесь используются только лишь русские фразы.
	//все же, чтобы не нарушать стиль оформления исходных кодов,
	//все фразы вынесены в отдельные константы

define('ADMIN_IMPORT_FROM_1C', 'Импорт из 1С');
define('ADMIN_1C_DESC1', 'Пожалуйста, введите имя экспортированного из 1С файла (списка номенклатуры).<br>Файл должен быть сохранен как `Текстовый файл` (с разделителями-табуляциями).<br> Пример можно посмотреть <a href="example_1c.txt">здесь</a>');
...
// still, for excel files fgetcsv is used...
{
	...
	if (isset($do_1c_import) && isset($filename) && isset($update_column)) //configuration finished - update database
	{
		...
		{
			echo "<p><b><font color=red>Не указано поле, по которому обновлять базу из файла</font></b>";
		...
		{
			echo "<p><b><font color=red>Поля название и код не заданы...</font></b>";
			//go to the previous step
			$proceed = 1;
			$file_1c = "";
			$file_1c_name = str_replace("products_pictures/","",$filename);
	...
	if (isset($proceed)) { //upload file and run configurator
		...
		if (isset($res) && $res) //file uploaded
		{
			...
			if ($data == -1) die ("Ошибка: не удалось прочитать файл");
				...
				<tr>
					...
					<td>
						<select name=db_association_$j>
							...
							<option value=\"product_code\"".mark_as_selected($data[$i][$j],"Код").">Код</option>
							<option value=\"name\"".mark_as_selected($data[$i][$j],"Наименование").">Наименование</option>
							<option value=\"Price\"".mark_as_selected($data[$i][$j],"Цена").">Цена</option>
							<option value=\"list_price\">Старая цена</option>
							<option value=\"in_stock\">Остаток на складе (шт.)</option>
							<option value=\"items_sold\">Продано (шт.)</option>
							<option value=\"description\">Описание</option>
							<option value=\"brief_description\">Краткое описание</option>
						</select>
					</td>
				</tr>
				";
			...
			//update column
			echo "<p>Колонка идентификации: <select name=update_column>";
				echo "<option value=\"product_code\">Код</option>";
				echo "<option value=\"name\">Наименование</option>";
				echo "<option value=\"Price\">Цена</option>";
			echo "</select><br>(укажите колонку, значение в которой однозначно идентифицирует товар)";
