...
If (CStr(Request.QueryString("GP_upload")) <> "") Then
  ...
  pau_thePath = """../../applications/PollingBoothManager/images"""
...
If (CStr(UploadFormRequest("MM_update")) <> "" And CStr(UploadFormRequest("MM_recordId")) <> "") Then
  ...
  MM_columnsStr = "QuestionImage|',none,''"
...
If (Request.QueryString("QuestionID")           <> "") Then 
  images__value1 = Request.QueryString("QuestionID")          
End If
%>
<%
set images = Server.CreateObject("ADODB.Recordset")
images.ActiveConnection = MM_pollingboothmanager_STRING
images.Source = "SELECT *  FROM tblPBM_PollQuestions  WHERE QuestionID = " + Replace(images__value1, "'", "''") + ""
images.CursorType = 0
...
<form ACTION="<%=MM_editAction%>" METHOD="post" enctype="multipart/form-data" name="imageA" id="imageA" onSubmit="checkFileUpload(this,'GIF,JPG,JPEG,BMP,PNG',true,'','','','','','','');return document.MM_returnValue">
  <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableborder">
    <tr valign="baseline"> 
      ...
      <td width="96%"> Upload Image: <strong>&nbsp; <%=(images.Fields.Item("QuestionValue").Value)%></strong></td>
    </tr>
    <tr valign="baseline"> 
...
<input type="hidden" name="MM_recordId" value="<%= images.Fields.Item("QuestionID").Value %>">
</form>
</body>
</html>
<%
