	...
	function doTextbox() {
		...
		oSel.pasteHTML("<table id=ew_textBox style='position:absolute;'><tr><td>Text box</td></tr></table>");

		textBox = foo.document.getElementById("ew_textBox")
	...
	function toggleTick(tick, state) {
		...
		if(tick.id.indexOf("zoom" + zoomSize + "_") > -1)
		{
			if(state == 1)
			{
				...
				tick.src = 'ew/ew_images/button_tick_inverted.gif'
			}
			else
			{
				// We are over the selected zoom
				tick.src = 'ew/ew_images/button_tick.gif'
			}
		}
	}
