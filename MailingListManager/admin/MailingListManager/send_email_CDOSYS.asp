...
<form action="cdo_mailerscript_CDOSYS.asp" method=post name="fHtmlEditor" id="fHtmlEditor">
		...
		<option value="1"> Use plain text format. </option>
                  ...
                  <tr bgcolor="#FFFFFF" >
					...
					<% If Not Request.QueryString("mode") = "html" Then %>					<textarea name="txtBody" cols="80" rows="12" id="txtBody">Enter Message
					</textarea>
					<%end if%>
  ...
  window.status  = "Current View: Wysiwyg";
				  ...
				  <tr bgcolor="#FFFFFF"> 
                    ...
                    <td height="2" colspan="2" align="left"> 
                      <input name="Submit" type="submit" onClick="OnFormSubmit()" value="Send to All Members">
					  <% If Request.QueryString("mode") = "html" Then %>
                      <input name="Reset" type="reset" onClick="newFile()" value="Reset Form">
					  <% else%>
					  <input type="reset" name="Submit3" value="Reset Form">
                      <% end if%>
                      <input name="Submit2" type="submit" onClick="OnFormSubmit()" value="Send Test Email">
