...
<table width="100%" cellpadding="0" cellspacing="0" class="body">
  ...
  <tr>
    <td align="right" class="body"><a href="admin.php?mode=ip_manager&adsess=<? echo $adsess; ?>">IP 
      Manager</a>&nbsp&nbsp<a href="admin.php?mode=banner_manager&act=create&adsess=<?=$adsess?>">Add a Banner</a>&nbsp;</td>
  </tr>
  <tr>
        ...
        <table width="98%" align="center" cellpadding="10" cellspacing="1" class="body">
          ...
          <?php
           while($cat=mysql_fetch_object($res)){
				...
				$ctr=round(($cat->b_clks/$cat->b_see)*100)."%";
  ...
  <tr> 
    <td colspan="2" valign="top" class="lined padded-6"> <form name="form1" action="admin.php" method="post" enctype="multipart/form-data">
        <table width="65%" align="center" cellpadding="10" cellspacing="1" class="body">
          ...
          <tr> 
            <td colspan="2" class="body">[<font size="1">Note: For Header dimensions 
              420x60 pixels. For Footer dimensions 720x60 pixels.</font>]</td>
          </tr>
          <tr> 
          ...
          <tr> 
            ...
            <td valign="top" class="body">From &nbsp;: 
              ...
              <select name=day_t id="day_t">
                <option selected value="0">Day 
                <? day_drop(0); ?>
              </select> 
              <select name=year_t id="year_t">
                <option selected value="0">Year 
                <? year_drop('now'); ?>
              </select></td>
          </tr>
...
"year_f","month_t","day_t","year_t","numi","numc");
...
if(empty($url) or empty($tmpfname) or empty($dur))	{
		...
		if(empty($blk))	$blk='N';
		$sql_ins="insert into banners (b_desc,b_url,b_typ,b_blk,b_dur,b_f_day,b_t_day,b_noi,b_ncl,b_dt) values (";
		$sql_ins.="'".addslashes($descr)."','".$url."','".$typ."','".$blk."','".$dur."','$f_day','$t_day','$numi','$numc',now())";
		mysql_query($sql_ins);
...
//			copy($tmpfname,$base_path."/banners/".$newname.$p_type);
			move_uploaded_file($tmpfname,$base_path."/banners/".$newname.$p_type);
			$photo="banners/".$newname.$p_type;
			$sql_img="update banners set b_img='".$photo."' where b_id=".$prim;
			mysql_query($sql_img);
		}
		$matt="this banner added to our list.";
    ...
    <td colspan="2" valign="top" class="lined padded-6"> <form name="form1" action="admin.php" method="post" enctype="multipart/form-data">
        <table width="65%" align="center" cellpadding="10" cellspacing="1" class="body">
          ...
          <tr align="center" valign="top"> 
            <td colspan="2" class="body"><?=$img_s?></td>
          </tr>
          <tr> 
            <td class="body">Type</td>
            <td class="body"><select name="typ" size="1" id="typ">
...
if(empty($url) or empty($dur))	{
		...
		$sql_up.="b_dur='".$dur."',b_f_day='$f_day',b_t_day='$t_day',b_noi='$numi',b_ncl='$numc',b_exp='N' where b_id=$seid";
		mysql_query($sql_up);
		$prim=$seid;
		if($tmpfname!='')	{
			if($ftype=="image/bmp")	$p_type=".bmp";
