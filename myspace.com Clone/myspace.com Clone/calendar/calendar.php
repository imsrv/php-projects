...
$query = "SELECT * FROM calendar_events WHERE event_month='$month' and event_year='$year' ORDER BY event_time";
...
<table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
		...
		for ($i = 1; $i <= $days_in_month; $i++) {
			...
			$link_month = $month;
			echo "<div align=\"right\">\n<a href=\"calendar.php?mode=event_add&day=$i&month=$link_month&year=$year\" style=\"text-decoration : none\">$i</a>&nbsp;</div>\n";
			if(isset($events[$i])){
				echo "<div align=\"left\">\n";
				while (list($key, $value) = each ($events[$i])) {
					$till=$event_info[$value]['1']+$event_info[$value]['2'];
					echo "&nbsp;<a href=\"calendar.php?mode=event&id=$value\" title=\"Duration : ".date ("H:i a", mktime($event_info[$value]['1'],0,0,$month,$date,$year))." - ".date ("H:i a", mktime($till,0,0,$month,$date,$year))."\">" . date ("H:i", mktime($event_info[$value]['1'],0,0,$month,$date,$year)) . "&nbsp;&raquo;&nbsp;" . $event_info[$value]['0']  . "</a>\n<br>\n";
				}
				echo "</div>\n";
			}
			echo "</td>\n";
