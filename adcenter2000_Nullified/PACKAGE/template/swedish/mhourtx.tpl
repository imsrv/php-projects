...
<tr><td width="100%" valign=center><font size=2 face="Verdana,Arial,Helvetica"><form action="$cgi/adcstat.pl" method=post><table><tr><td><input type=hidden name=lang value=$data{lang}><input type=hidden name=name value=$data{name}><input type=hidden name=password value=$data{password}><select name=method><option $selectede{tx} value=tx>Övergripande statistik</option><option $selectede{montlytx} value=montlytx>Månads Statisik</option><option $selectede{dailytx} value=dailytx>Daglig Statisik</option><option $selectede{hourlytx} value=hourlytx>Statisik per timme</option><option $selectede{mantx} value=mantx>Text</option><option $selectede{htmltx} value=htmltx>HTML kod</option></select></td><td><input type=image border=0 src="$adcenter/images/$data{lang}/goto.gif" width="70" height="23"></td></tr></table></form></font></td>
<td><form action="$cgi/adcstat.pl" method=post><table><tr><td><input type=hidden name=spot value=$data{spot}><input type=hidden name=name value=$data{name}><input type=hidden name=password value=$data{password}><input type=hidden name=method value=hourlytx><input type="hidden" name="lang" value="$data{lang}"><input type=text name=date size=2 value=$data{date}><select name=month><option $selected{Jan} value=Jan>Januari</option><option $selected{Feb} value=Feb>Februari</option><option $selected{Mar} value=Mar>Mars</option><option $selected{Apr} value=Apr>April</option><option $selected{Maj} value=May>Maj</option><option $selected{Jun} value=Jun>Juni</option><option $selected{Jul} value=Jul>Juli</option><option $selected{Aug} value=Aug>Augusti</option><option $selected{Sep} value=Sep>September</option><option $selected{Oct} value=Oct>Oktober</option><option $selected{Nov} value=Nov>November</option><option $selected{Dec} value=Dec>December</option></select><input type=text name=year value=$data{year} size=4></td><td><input type=image border=0 src="$adcenter/images/$data{lang}/getstats.gif" width="96" height="23"></td></tr></table></form></td></tr>
</table>
</td>
</tr>
</table>
...
<tr><td width="100%">
<font size=5 color="#000000">STATISTIK PER TIMME</font>
...
<tr><td colspan=2><center><font face="Lucida Console" size=1 color=red><br>Totala visningar under $data{date} $data{month} $data{year}: $totimp </font><br><br></center></td><td width=60 rowspan=3>&nbsp;</td></tr>
...
<div align=center><center><table width="346" bgcolor=white cellpadding=0 cellspacing=0 border=1><tr><td><table width="346" bgcolor=white cellpadding=0 cellspacing=0>
<tr><td colspan=2><center><font face="Lucida Console" size=1 color=red><br>Totala klick under $data{date} $data{month} $data{year}: $totclc </font><br><br></center></td><td width=60 rowspan=3>&nbsp;</td></tr>
...
<tr><td width="40%" valign="top"><font size=2><b>Tid</b></font></td><td width="30%" align="right"><font size=2><b>Visningar</b></font></td><td width="30%" align="right"><font size=2><b>Klic</b></font></td></tr>
