...
<tr><td width="100%">
<table border="0" width="100%">
<tr><td width="50%"><font size="4" face="Verdana,Arial,Helvetica"><b>SWIMBANNER MENU NAVIGATOR</b></font></td>
<td width="50%" align=right><font size="4" face="Verdana,Arial,Helvetica"><b>CHOOSE DATE</b></font></td></tr>
<tr><td width="100%" valign=center><font size=2 face="Verdana,Arial,Helvetica"><form action="$cgi/adcstat.pl" method=post><table><tr><td><input type=hidden name=lang value=$data{lang}><input type=hidden name=name value=$data{name}><input type=hidden name=password value=$data{password}><select name=method><option $selectede{str} value=str>Global stats</option><option $selectede{montlystr} value=montlystr>Monthly stats</option><option $selectede{dailystr} value=dailystr>Daily stats</option><option $selectede{hourlystr} value=hourlystr>Hourly stats</option><option $selectede{manstr} value=manstr>Manage Ads</option><option $selectede{htmlstr} value=htmlstr>Get HTML code</option></select></td><td><input type=image border=0 src="$adcenter/images/$data{lang}/goto.gif" width="70" height="23"></td></tr></table></form></font></td>
<td><form action="$cgi/adcstat.pl" method=post><table><tr><td><input type=hidden name=spot value=$data{spot}><input type=hidden name=name value=$data{name}><input type=hidden name=password value=$data{password}><input type=hidden name=method value=hourlystr><input type="hidden" name="lang" value="$data{lang}"><input type=text name=date size=2 value=$data{date}><select name=month><option $selected{Jan} value=Jan>January</option><option $selected{Feb} value=Feb>February</option><option $selected{Mar} value=Mar>March</option><option $selected{Apr} value=Apr>April</option><option $selected{May} value=May>May</option><option $selected{Jun} value=Jun>June</option><option $selected{Jul} value=Jul>July</option><option $selected{Aug} value=Aug>August</option><option $selected{Sep} value=Sep>September</option><option $selected{Oct} value=Oct>October</option><option $selected{Nov} value=Nov>November</option><option $selected{Dec} value=Dec>December</option></select><input type=text name=year value=$data{year} size=4></td><td><input type=image border=0 src="$adcenter/images/$data{lang}/getstats.gif" width="96" height="23"></td></tr></table></form></td></tr>
</table>
</td>
</tr>
</table>
