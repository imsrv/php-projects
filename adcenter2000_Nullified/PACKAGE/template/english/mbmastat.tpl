...
<tr><td>
<a href="$cgi/adcstat.pl?name=$data{name}&password=$data{password}&method=h_bmastat&lang=$data{lang}" target="_blank"><img src="$adcenter/images/$data{lang}/info.gif" border=0 alt="GET HELP"></a>&nbsp;&nbsp;<font size=5 color="#000000">UPLOAD GIF, JPG, JAVA, FLASH</font><br>
<font size=2><center>
<form action="$cgi/adcupl.pl" method="post" enctype="multipart/form-data">
<input type="hidden" name="name" value="$data{name}"><input type="hidden" name="lang" value="$data{lang}"><input type="hidden" name="password" value="$data{password}"><input type="hidden" name="spot" value="$data{spot}"><input type="hidden" name="method" value="$data{method}"><input type="file" name="UploadBanner" size="20"><input type="submit" value="Upload Banner"><br>Banner must be $banwidth[$data{spot}] x $banheight[$data{spot}] GIF, JPG, Java (class) or Flash (swl)<br>not more than $mfilesize[$data{spot}] bytes</form></center></font>
<font size=5 color="#000000">UPLOAD HTML BANNER</font><br>
<font size=2><center>
<form action="$cgi/adcstat.pl" method="post">
<input type="hidden" name="name" value="$data{name}"><input type="hidden" name="lang" value="$data{lang}"><input type="hidden" name="password" value="$data{password}"><input type="hidden" name="spot" value="$data{spot}"><input type="hidden" name="method" value="uplhtml"><textarea name="file" cols="44" rows=6></textarea><br><input type="submit" value="Upload Banner"><br>Banner must be $banwidth[$data{spot}] x $banheight[$data{spot}]</form></center></font>
