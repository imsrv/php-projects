...
print "<body>\n";
print "\n";
print "<p align=\"center\"><font face=\"Verdana\">Nullified by CyKuH [WTN]\n";
print "\n";
print "<p align=\"center\"><font face=\"Verdana\"><b>Please wait, setup is attempting to\n";
print "create tables in your MySQL database.</b></font></p>\n";
print "\n";
...
print "DB name correctly in config.pl!</b><!--CyKuH [WTN]--></font></p>\n";
...
my($query) = "CREATE TABLE DMtgpblind (
   ...
   thumb char(1) DEFAULT '0' NOT NULL 
   PRIMARY KEY (idnum),
   KEY webdesc (webdesc)
);";
        my($sth) = $dbh->prepare($query);
...
my($query) = "INSERT INTO DMtgpcategories VALUES ( 'Brunettes', 'Brunettes', '1');";
        my($sth) = $dbh->prepare($query);
        $sth->execute() or die "Unable to execute query: ".$sth->errstr;
        $sth->finish;
my($query) = "INSERT INTO DMtgpcategories VALUES ( 'Blondes', 'Blondes', '1');";
        my($sth) = $dbh->prepare($query);
        $sth->execute() or die "Unable to execute query: ".$sth->errstr;
        $sth->finish;
my($query) = "INSERT INTO DMtgpcategories VALUES ( 'Amateurs', 'Amateurs', '1');";
        my($sth) = $dbh->prepare($query);
        $sth->execute() or die "Unable to execute query: ".$sth->errstr;
        $sth->finish;
my($query) = "INSERT INTO DMtgpcategories VALUES ( 'Asians', 'Asians', '1');";
        my($sth) = $dbh->prepare($query);
        $sth->execute() or die "Unable to execute query: ".$sth->errstr;
        $sth->finish;
my($query) = "INSERT INTO DMtgpcategories VALUES ( 'Anal Sex', 'Anal', '1');";
        my($sth) = $dbh->prepare($query);
        $sth->execute() or die "Unable to execute query: ".$sth->errstr;
        $sth->finish;
my($query) = "INSERT INTO DMtgpcategories VALUES ( 'Oral Sex', 'Oral', '1');";
        my($sth) = $dbh->prepare($query);
        $sth->execute() or die "Unable to execute query: ".$sth->errstr;
        $sth->finish;
my($query) = "INSERT INTO DMtgpcategories VALUES ( 'Cumshots', 'Cumshots', '1');";
        my($sth) = $dbh->prepare($query);
        $sth->execute() or die "Unable to execute query: ".$sth->errstr;
        $sth->finish;
my($query) = "INSERT INTO DMtgpcategories VALUES ( 'Dicks/Shemales', 'DicksandShemales', '1');";
        my($sth) = $dbh->prepare($query);
        $sth->execute() or die "Unable to execute query: ".$sth->errstr;
        $sth->finish;
my($query) = "INSERT INTO DMtgpcategories VALUES ( 'Fetish/Bondage/Watersports', 'FetishBondageWatersports', '1');";
        my($sth) = $dbh->prepare($query);
        $sth->execute() or die "Unable to execute query: ".$sth->errstr;
        $sth->finish;
my($query) = "INSERT INTO DMtgpcategories VALUES ( 'Babes', 'Babes', '1');";
        my($sth) = $dbh->prepare($query);
        $sth->execute() or die "Unable to execute query: ".$sth->errstr;
        $sth->finish;
my($query) = "INSERT INTO DMtgpcategories VALUES ( 'Groups', 'Groups', '1');";
        my($sth) = $dbh->prepare($query);
        $sth->execute() or die "Unable to execute query: ".$sth->errstr;
        $sth->finish;
my($query) = "INSERT INTO DMtgpcategories VALUES ( 'Hardcore', 'Hardcore', '1');";
        my($sth) = $dbh->prepare($query);
        $sth->execute() or die "Unable to execute query: ".$sth->errstr;
        $sth->finish;
my($query) = "INSERT INTO DMtgpcategories VALUES ( 'Interracial', 'Interracial', '1');";
        my($sth) = $dbh->prepare($query);
        $sth->execute() or die "Unable to execute query: ".$sth->errstr;
        $sth->finish;
my($query) = "INSERT INTO DMtgpcategories VALUES ( 'Cheerleaders/Uniforms', 'CheerleadersUniforms', '1');";
        my($sth) = $dbh->prepare($query);
        $sth->execute() or die "Unable to execute query: ".$sth->errstr;
        $sth->finish;
my($query) = "INSERT INTO DMtgpcategories VALUES ( 'Legal Teens', 'Teens', '1');";
        my($sth) = $dbh->prepare($query);
        $sth->execute() or die "Unable to execute query: ".$sth->errstr;
        $sth->finish;
my($query) = "INSERT INTO DMtgpcategories VALUES ( 'Lesbians', 'Lesbians', '1');";
        my($sth) = $dbh->prepare($query);
        $sth->execute() or die "Unable to execute query: ".$sth->errstr;
        $sth->finish;
my($query) = "INSERT INTO DMtgpcategories VALUES ( 'Voyeur/Exhibitionists/Upskirts', 'VoyeurExhibitionistsUpskirts', '1');";
        my($sth) = $dbh->prepare($query);
        $sth->execute() or die "Unable to execute query: ".$sth->errstr;
        $sth->finish;
my($query) = "INSERT INTO DMtgpcategories VALUES ( 'Fat Women (BBW)', 'Fat', '1');";
        my($sth) = $dbh->prepare($query);
        $sth->execute() or die "Unable to execute query: ".$sth->errstr;
        $sth->finish;
my($query) = "INSERT INTO DMtgpcategories VALUES ( 'Redheads', 'Redheads', '1');";
        my($sth) = $dbh->prepare($query);
        $sth->execute() or die "Unable to execute query: ".$sth->errstr;
        $sth->finish;
my($query) = "INSERT INTO DMtgpcategories VALUES ( 'Celebrities', 'Celebrities', '1');";
        my($sth) = $dbh->prepare($query);
        $sth->execute() or die "Unable to execute query: ".$sth->errstr;
        $sth->finish;
my($query) = "INSERT INTO DMtgpcategories VALUES ( 'Older Women', 'Older', '1');";
        my($sth) = $dbh->prepare($query);
        $sth->execute() or die "Unable to execute query: ".$sth->errstr;
        $sth->finish;
my($query) = "INSERT INTO DMtgpcategories VALUES ( 'Sex Toys', 'Toys', '1');";
        my($sth) = $dbh->prepare($query);
        $sth->execute() or die "Unable to execute query: ".$sth->errstr;
        $sth->finish;
my($query) = "INSERT INTO DMtgpcategories VALUES ( 'Pregnant', 'Pregnant', '1');";
        my($sth) = $dbh->prepare($query);
        $sth->execute() or die "Unable to execute query: ".$sth->errstr;
        $sth->finish;
my($query) = "INSERT INTO DMtgpcategories VALUES ( 'Blondes', 'Blondes', '1');";
        ...
        $sth->execute() or die "Unable to execute query: ".$sth->errstr;
        $sth->finish;
my($query) = "INSERT INTO DMtgpcategories VALUES ( 'Videos/MPEGs/AVIs', 'Video', '1');";
        my($sth) = $dbh->prepare($query);
        $sth->execute() or die "Unable to execute query: ".$sth->errstr;
        $sth->finish;
my($query) = "INSERT INTO DMtgpcategories VALUES ( 'Gays', 'Gays', '1');";
        my($sth) = $dbh->prepare($query);
        $sth->execute() or die "Unable to execute query: ".$sth->errstr;
        $sth->finish;
...
print "    <tr>\n";
print "      <td width=\"100%\">\n";
print "        <p align=\"center\"><font face=\"Verdana\"><b>Tables were created!</b><!--CyKuH [WTN]--></font></p>\n";
print "        <p align=\"center\"><b><i><font face=\"Verdana\" size=\"4\">You will not need\n";
print "        setup.cgi again!</font></i></b><font face=\"Verdana\"><b><br>\n";
