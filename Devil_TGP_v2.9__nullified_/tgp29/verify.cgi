...
$dbh = DBI->connect("dbi:mysql:$database:$dbhost:$dbport","$user","$pass") || die("Can not connect to mySQL database!\n");
	my($query) = "SELECT * FROM DMtgpgalleries WHERE uniqueid = '$postidnum' and vermail = '0'";
...
print "either does not exist or has already been confirmed<!--CyKuH [WTN]--></font></b></p>\n";
exit;
