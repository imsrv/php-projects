...
If (CStr(UploadFormRequest("MM_update")) <> "" And CStr(UploadFormRequest("MM_recordId")) <> "") Then
  ...
  MM_columnsStr = "ImageFileA3|',none,''"
...
%>
<%
Dim imageA3__MMColParam
imageA3__MMColParam = "0"
If (Request.QueryString("mid3")   <> "") Then 
  imageA3__MMColParam = Request.QueryString("mid3")  
End If
%>
<%
set imageA3 = Server.CreateObject("ADODB.Recordset")
imageA3.ActiveConnection = MM_sitechassismanager_STRING
imageA3.Source = "SELECT tblSitePlanNavMenu.*, tblSitePlanNavMenu2.*, tblSitePlanNavMenu3.*  FROM (tblSitePlanNavMenu LEFT JOIN tblSitePlanNavMenu2 ON tblSitePlanNavMenu.mid = tblSitePlanNavMenu2.midkey) LEFT JOIN tblSitePlanNavMenu3 ON tblSitePlanNavMenu2.mid2 = tblSitePlanNavMenu3.mid2key  WHERE mid3 = " + Replace(imageA3__MMColParam, "'", "''") + ""
imageA3.CursorType = 0
imageA3.CursorLocation = 2
imageA3.LockType = 3
imageA3.Open()
imageA3_numRows = 0
%>
...
<form ACTION="<%=MM_editAction%>" METHOD="post" name="form1" enctype="multipart/form-data" onSubmit="checkFileUpload(this,'GIF,JPG,JPEG,BMP,PNG');checkFileUpload(this,'GIF,JPG,JPEG,BMP,PNG',true,'','','','','','','');return document.MM_returnValue">
  <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableborder">
    <tr valign="baseline"> 
      ...
      <td width="96%"> Image will be added to the following page: <strong><%=(imageA3.Fields.Item("Menu").Value)%>
	  
        <% if imageA3.Fields.Item("Menu2").Value <> "" then %>
	   - <%=(imageA3.Fields.Item("Menu2").Value)%> 
	   <% end if%>
	   <% if imageA3.Fields.Item("Menu3").Value <> "" then %>
	   - <%=(imageA3.Fields.Item("Menu3").Value)%>
	   <% end if%> 
	   </strong></td>
    </tr>
    <tr valign="baseline"> 
  ...
  <input type="hidden" name="MM_recordId" value="<%= imageA3.Fields.Item("mid3").Value %>">
</form>
</body>
</html>
<%
