...
If (CStr(UploadFormRequest("MM_update")) <> "" And CStr(UploadFormRequest("MM_recordId")) <> "") Then
  ...
  MM_columnsStr = "ImageFileB3|',none,''"
...
%>
<%
Dim imageB3__MMColParam
imageB3__MMColParam = "0"
If (Request.QueryString("mid3")   <> "") Then 
  imageB3__MMColParam = Request.QueryString("mid3")  
End If
%>
<%
set imageB3 = Server.CreateObject("ADODB.Recordset")
imageB3.ActiveConnection = MM_sitechassismanager_STRING
imageB3.Source = "SELECT tblSitePlanNavMenu.*, tblSitePlanNavMenu2.*, tblSitePlanNavMenu3.*  FROM (tblSitePlanNavMenu LEFT JOIN tblSitePlanNavMenu2 ON tblSitePlanNavMenu.mid = tblSitePlanNavMenu2.midkey) LEFT JOIN tblSitePlanNavMenu3 ON tblSitePlanNavMenu2.mid2 = tblSitePlanNavMenu3.mid2key  WHERE mid3 = " + Replace(imageB3__MMColParam, "'", "''") + ""
imageB3.CursorType = 0
imageB3.CursorLocation = 2
imageB3.LockType = 3
imageB3.Open()
imageB3_numRows = 0
%>
...
<body>
<form ACTION="<%=MM_editAction%>" METHOD="post" name="form1" enctype="multipart/form-data" onSubmit="checkFileUpload(this,'GIF,JPG,JPEG,BMP,PNG');checkFileUpload(this,'GIF,JPG,JPEG,BMP,PNG',true,'','','','','','','');return document.MM_returnValue">
  <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableborder">
    <tr valign="baseline"> 
      <td width="4%" align="right" nowrap>&nbsp;</td>
      <td width="96%"> Image will be added to the following page: <strong><%=(imageB3.Fields.Item("Menu").Value)%>
	  
        <% if imageB3.Fields.Item("Menu2").Value <> "" then %>
	   - <%=(imageB3.Fields.Item("Menu2").Value)%> 
	   <% end if%>
	   <% if imageB3.Fields.Item("Menu3").Value <> "" then %>
	   - <%=(imageB3.Fields.Item("Menu3").Value)%>
	   <% end if%> 
	   </strong></td>
    </tr>
    <tr valign="baseline"> 
  ...
  <input type="hidden" name="MM_recordId" value="<%= imageB3.Fields.Item("mid3").Value %>">
</form>
</body>
</html>
<%
