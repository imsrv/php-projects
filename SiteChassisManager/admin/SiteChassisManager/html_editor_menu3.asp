...
If (CStr(Request("MM_update")) = "fHtmlEditor" And CStr(Request("MM_recordId")) <> "") Then
  ...
  MM_columnsStr = "SectionContent3|',none,''"
...
If (Request.QueryString("mid3")      <> "") Then 
  menuedithtml__value1 = Request.QueryString("mid3")     
End If
%>
<%
set menuedithtml = Server.CreateObject("ADODB.Recordset")
menuedithtml.ActiveConnection = MM_sitechassismanager_STRING
menuedithtml.Source = "SELECT *  FROM tblSitePlanNavMenu3  WHERE mid3 = " + Replace(menuedithtml__value1, "'", "''") + ""
...
<form name="fHtmlEditor" method="POST" action="<%=MM_editAction%>">
  <textarea name="EditorValue" style="display: none;"><%=(menuedithtml.Fields.Item("SectionContent3").Value)%></textarea>
                  ...
                  <tr> 
                    <td height="27"> <table width="100%" border="0" cellpadding="0" cellspacing="0">
                        <tr> 
                          ...
                          <td><a href="update_menu3.asp?mid3=<%=(menuedithtml.Fields.Item("mid3").Value)%>">Edit Menu Item Details</a></td>
                        </tr>
                    </table></td>
                  </tr>
                  <tr> 
...
%>
  ...
  <input type="hidden" name="MM_recordId" value="<%= menuedithtml.Fields.Item("mid3").Value %>">
</form>
<script>
  initToolBar("foo");
