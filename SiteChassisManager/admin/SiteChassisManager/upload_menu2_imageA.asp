...
If (CStr(UploadFormRequest("MM_update")) <> "" And CStr(UploadFormRequest("MM_recordId")) <> "") Then
  ...
  MM_columnsStr = "ImageFileA2|',none,''"
...
%>
<%
Dim imageA2__MMColParam
imageA2__MMColParam = "0"
If (Request.QueryString("mid2")   <> "") Then 
  imageA2__MMColParam = Request.QueryString("mid2")  
End If
%>
<%
set imageA2 = Server.CreateObject("ADODB.Recordset")
imageA2.ActiveConnection = MM_sitechassismanager_STRING
imageA2.Source = "SELECT tblSitePlanNavMenu.*, tblSitePlanNavMenu2.*, tblSitePlanNavMenu3.*  FROM (tblSitePlanNavMenu LEFT JOIN tblSitePlanNavMenu2 ON tblSitePlanNavMenu.mid = tblSitePlanNavMenu2.midkey) LEFT JOIN tblSitePlanNavMenu3 ON tblSitePlanNavMenu2.mid2 = tblSitePlanNavMenu3.mid2key  WHERE mid2 = " + Replace(imageA2__MMColParam, "'", "''") + ""
imageA2.CursorType = 0
imageA2.CursorLocation = 2
imageA2.LockType = 3
imageA2.Open()
imageA2_numRows = 0
%>
...
<form ACTION="<%=MM_editAction%>" METHOD="post" name="form1" enctype="multipart/form-data" onSubmit="checkFileUpload(this,'GIF,JPG,JPEG,BMP,PNG');checkFileUpload(this,'GIF,JPG,JPEG,BMP,PNG',true,'','','','','','','');return document.MM_returnValue">
  <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableborder">
    <tr valign="baseline"> 
      ...
      <td width="96%"> Image will be added to the following page: <strong><%=(imageA2.Fields.Item("Menu").Value)%>
	  
        <% if imageA2.Fields.Item("Menu2").Value <> "" then %>
	   - <%=(imageA2.Fields.Item("Menu2").Value)%> 
	   <% end if%>
	   <% if imageA2.Fields.Item("Menu3").Value <> "" then %>
	   - <%=(imageA2.Fields.Item("Menu3").Value)%>
	   <% end if%> 
	   </strong></td>
    </tr>
    <tr valign="baseline"> 
  ...
  <input type="hidden" name="MM_recordId" value="<%= imageA2.Fields.Item("mid2").Value %>">
</form>
</body>
</html>
<%
