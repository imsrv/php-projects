		...
		if ($error){
			...
			if ($attachments>0){
				for ($i=0;$i<$attachments;$i++){
					...
					if ($a_attach==1){
						...
						$fp=fopen($a_path,"r");
						$a_data=fread($fp,filesize($a_path));
						fclose($fp);
			...
			if (mail($to,$subject,$body,$headerx, "-f$from_addr")){
				...
				if ($my_prefs["save_sent"]==1){
					$append_str="To: ".$to."\n"."Subject: ".$subject."\n".$headerx."\n\n".$body;
					iil_C_Append($conn, $my_prefs["sent_box_name"], $append_str);
				}
...
<FORM ENCTYPE="multipart/form-data" ACTION="compose.php" METHOD=POST>
	...
	<br><input type=checkbox name="attach_sig" value=1 <? echo ($my_prefs["show_sig1"]==1?"CHECKED":""); ?> > 		<? echo $composeStrings[3]; ?>
	<?
	if ($attachments>0){
		for ($i=0;$i<$attachments;$i++){
			$var="attachment_path_".$i;
