...
<?
 ...
 if (!defined('_MMDB_LAYER')) {
 	define('_MMDB_LAYER',1);
	
	define('MMDB_BAD_RS','<p>Bad $rs in %s. Connection or SQL invalid. Try using $connection->debug=true;</p>');
	
	define('MMDB_FETCH_DEFAULT',0);
	define('MMDB_FETCH_NUM',1);
	define('MMDB_FETCH_ASSOC',2);
	define('MMDB_FETCH_BOTH',3);
	
	GLOBAL 
		$MMDB_vers, 		// database version
		$MMDB_Database, 	// last database driver used
		$MMDB_COUNTRECS,	// count number of records returned - slows down query
		$MMDB_CACHE_DIR,	// directory to cache recordsets
	 	$MMDB_FETCH_MODE;	// DEFAULT, NUM, ASSOC or BOTH. Default follows native driver default...
	
	$MMDB_FETCH_MODE = MMDB_FETCH_DEFAULT;
	/** 
	 * SET THE VALUE BELOW TO THE DIRECTORY WHERE THIS FILE RESIDES
	 * MMDB_RootPath has been renamed MMDB_DIR 
	 */
	if (!defined('MMDB_DIR')) define('MMDB_DIR',dirname(__FILE__));
	
	if (!isset($MMDB_CACHE_DIR)) $MMDB_CACHE_DIR = '/tmp';
	else if (strpos($MMDB_CACHE_DIR,':/') !== false) die("Illegal \$MMDB_CACHE_DIR");
	
	//==============================================================================================	
	// CHANGE NOTHING BELOW UNLESS YOU ARE CODING
	//==============================================================================================	

	// using PHPCONFIG protocol overrides MMDB_DIR
	//if (isset($PHPCONFIG_DIR)) $MMDB_DIR=$PHPCONFIG_DIR.'/../adodb';
	...
	/**
	 * MMDB version as a string.
	 */
	$MMDB_vers = 'V1.54 15 Nov 2001 (c) 2000, 2001 John Lim. All rights reserved. Released BSD & LGPL.';
	...
	var $_bindInputArray = false; // set to true if MMConnection.Execute() permits binding of array parameters.
	
	/**
	 * Constructor
	 */
	function MMConnection()			
	{
	...
	*/
	...
	{
	global $MMDB_FETCH_MODE;
		$MMDB_FETCH_MODE = $mode;
	}
	...
	*/
	function &Query($sql, $inputarr=false)
	{
		...
		if (!$rs && defined('MMDB_PEAR')) return MMDB_PEAR_Error();
		return $rs;
	}
	...
	*/
	...
	function &Execute($sql,$inputarr=false,$arg3=false) 
	{
		...
		if ($this->_queryID === false) {
			...
			$rs = new MMRecordSet_empty();
			return $rs;
		}
		$rsclass = "MMRecordSet_".$this->databaseType;
	...
	* @return		the recordset ($rs->databaseType == 'array')
	...
	function &SelectLimit($sql,$nrows=-1,$offset=-1, $inputarr=false,$arg3=false,$secs2cache=0)
	{
		...
		if ($offset <= 0) {
			...
			if ($nrows > 0) {
					$sql = "select * from ($sql) where rownum <= :adodb_rownum";
	...
	*/
	function GetAll($sql,$inputarr=false)
	{
		$rs = $this->Execute($sql,$inputarr);
		if (!$rs) 
			if (defined('MMDB_PEAR')) return MMDB_PEAR_Error();
			else return false;
		return $rs->GetArray();
	}
	...
	* @return		the recordset ($rs->databaseType == 'array')
	...
	{
	global $MMDB_CACHE_DIR;
	
		return $MMDB_CACHE_DIR.'/adodb_'.md5($sql.$this->databaseType.$this->database.$this->user).'.cache';
	}
	/**
	...
	{
		if (!$rs) {
			printf(MMDB_BAD_RS,'GetUpdateSQL');
			return false;
		}
	...
	{
		if (!$rs) {
			printf(MMDB_BAD_RS,'GetInsertSQL');
			return false;
		}
		// Get the table name from the existing query.
		eregi("FROM ([0-9a-z_-]*)", $rs->sql, $tableName);
	...
	*/
	...
	/**
	 * List columns in a database as an array of MMFieldObjects. 
	 * See top of file for definition of object.
	 *
	 * @params table	table name to query
	 *
	 * @return  array of MMFieldObjects for current table.
	 */ 
...
function &PageExecute($sql, $nrows, $page, $inputarr=false, $arg3=false, $secs2cache=0) 
{
	...
	*/
	 ...
	 * Constructor
	 *
	 * @param queryID  	this is the queryID returned by MMConnection->_query()
	 *
	 */
	function MMRecordSet(&$queryID) 
	{
	 ...
	 * @return HTML
	 *
	 * support multiple hilited items
	 */
	...
	{
		return MMRecordSet::GetMenu($name,$defstr,$blank1stItem,$multiple,$size, $selectAttr,false);
	}
	...
	/**
	 * return recordset as a 2-dimensional array. 
	 * Helper function for MMConnection->SelectLimit()
	 *
	 * @param offset	is the row to start calculations from (1-based)
	...
	/**
	 * Synonym for GetArray() for compatibility with MM.
	 *
	 * @param [nRows]  is the number of rows to return. -1 means every row.
	 *
	 * @return an array indexed by the rows (0-based) from the recordset
	...
	*/
	 ...
	 * Will not work if MMDB_FETCH_MODE is set to MMDB_FETCH_NUM.
	 * 
   ...
   *
   * If you don't want uppercase cols, set $MMDB_FETCH_MODE = MMDB_FETCH_ASSOC
   ...
   */
	...
	/**
	 * synonym for CurrentRow -- for MM compat
	 *
	 * @return the current row in the recordset. If at EOF, will return the last row. 0-based.
	 */
	...
	/**
	 * Get the MMFieldObject of a specific column.
	 *
	 * @param fieldoffset	is the column position to access(0-based).
	 *
	 * @return the MMFieldObject for that column, or false.
	 */
	...
	*/
	 ...
	 *
	 * @param t  is the type passed in. Normally is MMFieldObject->type.
	...
	*/
	...
	class MMRecordSet_array extends MMRecordSet
	{
		 ...
		 *
		 */
		function MMRecordSet_array($fakeid=1)
		{
			$this->MMRecordSet($fakeid); // fake queryID
		}
		 ...
		 *			The first row should hold the column names 
		 *			unless paramter $colnames is used.
		 * @param fieldarr	holds an array of MMFieldObject's.
		 */
    ...
    /**
	 * Synonym for MMLoadCode.
	 *
	 * @deprecated
	 */
	function MMLoadDB($dbType) { return MMLoadCode($dbType);}
        
    /**
	 * Load the code for a specific database driver
	 */
    function MMLoadCode($dbType) 
	{
	GLOBAL $MMDB_Database;
	
		if (!$dbType) return false;
		$MMDB_Database = strtolower($dbType);
		switch ($MMDB_Database) {
			case 'maxsql': $MMDB_Database = 'mysqlt'; break;
			case 'pgsql': $MMDB_Database = 'postgres7'; break;
		}
if (! defined("_MMDB_MYSQL_LAYER")) {
 define("_MMDB_MYSQL_LAYER", 1 );

class MMDB_mysql extends MMConnection {
	...
	{
	global $MMDB_COUNTRECS;
		if($MMDB_COUNTRECS) return mysql_query($sql,$this->_connectionID);
...
class MMRecordSet_mysql extends MMRecordSet{	
	
	var $databaseType = "mysql";
	var $canSeek = true;
	
	function MMRecordSet_mysql($queryID) {
	GLOBAL $MMDB_FETCH_MODE;
	
		switch ($MMDB_FETCH_MODE)
		{
		case MMDB_FETCH_NUM: $this->fetchMode = MYSQL_NUM; break;
		case MMDB_FETCH_ASSOC:$this->fetchMode = MYSQL_ASSOC; break;
		default:
		case MMDB_FETCH_DEFAULT:
		case MMDB_FETCH_BOTH:$this->fetchMode = MYSQL_BOTH; break;
		}
	
		$this->MMRecordSet($queryID);	
	}
	
	function _initrs()
	{
	GLOBAL $MMDB_COUNTRECS;
		$this->_numOfRows = ($MMDB_COUNTRECS) ? @mysql_num_rows($this->_queryID):-1;
	...
	function &FetchField($fieldOffset = -1) {
		if ($fieldOffset != -1) {
			...
			$o->max_length = @mysql_field_len($this->_queryID,$fieldOffset); // suggested by: Jim Nicholson
			//$o->max_length = -1; // mysql returns the max length less spaces -- so it is unrealiable
			$o->binary = (strpos($f,'binary')!== false);
		}
		else if ($fieldOffset == -1) {	/*	The $fieldOffset argument is not provided thus its -1 	*/
			$o = @mysql_fetch_field($this->_queryID);
			$o->max_length = @mysql_field_len($this->_queryID); // suggested by: Jim Nicholson
			//$o->max_length = -1; // mysql returns the max length less spaces -- so it is unrealiable
		}
	...
	function &GetRowAssoc($upper=true)
	{
		...
		return MMRecordSet::GetRowAssoc($upper);
	}
	...
	/**
	 * synonym for MMNewConnection for people who cannot remember the correct name
	 */
	function &NewMMConnection($db='')
	{
		return MMNewConnection($db);
	}
	 ...
	 * 	use the last database driver that was loaded by MMLoadCode().
	 *
	 * @return the freshly created instance of the Connection class.
	 */
	function &MMNewConnection($db='')
	{
	GLOBAL $MMDB_Database;
	
		if ($db) {
			if ($MMDB_Database != $db) MMLoadCode($db);
		} else { 
			if (!empty($MMDB_Database)) $db = $MMDB_Database;
			else print "<p>MMNewConnection: No database driver defined</p>";
		}
		
		$cls = 'MMDB_'.$db;
		$obj = new $cls();
		if (defined('MMDB_ERROR_HANDLER')) {
			$obj->raiseErrorFn = MMDB_ERROR_HANDLER;
		}
		return $obj;
	}
...
$db = &MMNewConnection($db_login_info->db_type);
$db->PConnect($db_login_info->db_addr, $db_login_info->db_user, $db_login_info->db_pwd, $db_login_info->db_name);
