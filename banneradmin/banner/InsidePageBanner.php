<?php
   $DB_SERVER="xxxxx";      // Database Server machine
   $DB_LOGIN="xxxxx";                     // Database login
   $DB_PASSWORD="xxxxx";                  // Database password
   $DB="xxxxx";                         // Database containing the tables
   $HTTP_HOST="http://xxxxxx";      // HTTP Host
...
$db = mysql_select_db($DB, $connection)
 ...
 if(!isset($bannerID)){
       $query =  "select bannerID, banner, src, alt, width, height from banners WHERE local_banner='2' AND active='true'ORDER by RAND()";
