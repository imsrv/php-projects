...
# The program also checks to make sure that it has not been run recently,
# based on the value that you set for $autonotify_purge_run_interval.
# That way, no one can accidentally (or
# intentionally) run this program repeatedly and cause it to bog down your
# server or risk corruption of your data.
...
$form_data{'autonotify_purge_button'} = "on";
