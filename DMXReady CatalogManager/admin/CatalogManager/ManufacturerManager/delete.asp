...
If (Request.QueryString("manid")       <> "") Then 
  deletelistitem__MMColParam = Request.QueryString("manid")      
End If
%>
<%
set deletelistitem = Server.CreateObject("ADODB.Recordset")
deletelistitem.ActiveConnection = MM_catalogmanager_STRING
deletelistitem.Source = "SELECT *  FROM tblManufacturers  WHERE ManufacturerID = " + Replace(deletelistitem__MMColParam, "'", "''") + ""
...
<input type="hidden" name="MM_recordId" value="<%= deletelistitem.Fields.Item("ManufacturerID").Value %>">
</form>
<%
