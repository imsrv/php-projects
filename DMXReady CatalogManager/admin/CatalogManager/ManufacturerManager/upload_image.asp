...
If (CStr(UploadFormRequest("MM_update")) <> "" And CStr(UploadFormRequest("MM_recordId")) <> "") Then
  ...
  MM_columnsStr = "ManufacturerImageFile|',none,''"
...
If (Request.QueryString("manid")     <> "") Then 
  images__value1 = Request.QueryString("manid")    
End If
%>
<%
set images = Server.CreateObject("ADODB.Recordset")
images.ActiveConnection = MM_catalogmanager_STRING
images.Source = "SELECT *  FROM tblManufacturers  WHERE ManufacturerID = " + Replace(images__value1, "'", "''") + ""
images.CursorType = 0
...
<form ACTION="<%=MM_editAction%>" METHOD="post" enctype="multipart/form-data" name="imageA" id="imageA" onSubmit="checkFileUpload(this,'GIF,JPG,JPEG,BMP,PNG',true,'','','','','','','');return document.MM_returnValue">
  <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableborder">
    <tr valign="baseline"> 
      ...
      <td width="96%"> Upload Image: <strong><%=(images.Fields.Item("Manufacturer").Value)%></strong></td>
    </tr>
    <tr valign="baseline"> 
...
<input type="hidden" name="MM_recordId" value="<%= images.Fields.Item("ManufacturerID").Value %>">
</form>
</body>
</html>
<%
