...
If (Request.QueryString("gpcid")          <> "") Then 
  deletelistitem__value1 = Request.QueryString("gpcid")         
End If
%>
<%
set deletelistitem = Server.CreateObject("ADODB.Recordset")
deletelistitem.ActiveConnection = MM_catalogmanager_STRING
deletelistitem.Source = "SELECT *  FROM tblGPC  WHERE GPCID = " + Replace(deletelistitem__value1, "'", "''") + ""
...
<form ACTION="<%=MM_editAction%>" METHOD="POST" name="form1">
  ...
  <input type="hidden" name="MM_recordId" value="<%= deletelistitem.Fields.Item("GPCID").Value %>">
</form>
<%
