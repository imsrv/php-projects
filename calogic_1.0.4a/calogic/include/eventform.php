...
function neweventform($cuser,$viewdate,$viewtype,$func,$funcdate,$functime) {
...
//prepare some field output
    ...
    $fyear=substr($funcdate,0,4);
    $fmonth=substr($funcdate,4,2);
    $fday=substr($funcdate,6,2);
    $fhour=substr($functime,0,2);
    $fmin=substr($functime,2,2);
...
<input type="hidden" name="nef[alldayevent]" id="alldayevent" value="0" />
...
<div title="Enter the Title of the event here. It will appear on the different Calendar views.">
    <b>Title:</b>
    <input title="Enter the Title of the event here. It will appear on the different Calendar views." name="nef[eventtitle]" size="32" id="eventtitle" maxLength="50">
</div>
    </td>
    <td width="70%">
...
<?php
    for($lc=0;$lc<47;$lc++) {
		echo "        <option ";
        if(substr($curcalcfg["daybeginhour"],0,2)==substr($timear[$lc],0,2)) {
            echo "selected ";
        }
...
<?php
    for($lc=0;$lc<=59;$lc++) {
        if($lc < 10) {$mlc = "0".$lc;} else {$mlc=$lc;}
		echo "        <option ";
        if(substr($curcalcfg["daybeginhour"],2,2)==$mlc) {
            echo "selected ";
        }
        echo "value = \"".$mlc."\" >".$mlc."</option>\n";
     }
...
<?php
    for($lc=0;$lc<47;$lc++) {
		echo "        <option ";
        if(substr($curcalcfg["dayendhour"],0,2)==substr($timear[$lc],0,2)) {
            echo "selected ";
        }
        echo "value = \"".substr($timear[$lc],0,2)."\" >";
        if($curcalcfg["timetype"]==1) {
...
<?php
    for($lc=0;$lc<=59;$lc++) {
        if($lc < 10) {$mlc = "0".$lc;} else {$mlc=$lc;}
		echo "        <option ";
        if(substr($curcalcfg["dayendhour"],2,2)==$mlc) {
            echo "selected ";
        }
        echo "value = \"".$mlc."\" >".$mlc."</option>\n";
     }
...
<div title="Enter a description for the event. This description also gets sent with reminder mails.">
        ...
        <tr>
          <td width="100%">
          <TEXTAREA title="Enter a description for the event. This description also gets sent with reminder mails." id="desc" style="WIDTH: 272px; HEIGHT: 117px" name="nef[desc]" rows="6" cols="31"></TEXTAREA>
          </td>
        </tr>
        <tr>
          <td width="100%">
...
?>
          <hr size="1" />
          <INPUT title="Check this box to send a reminder for this event." type="checkbox" id="sendreminder" name="nef[sendreminder]" LANGUAGE=javascript onclick="return sendreminder_onclick()">
          <label title="Check this box to send a reminder for this event." for="sendreminder"><b>Send Reminder</b></label>
<div style="display: none" id="reminderbox">
...
<div style="display: inline" title="Add contacts to the reminder list by double clicking the entry.">
            ...
            <option value="M0" text="Myself">Myself</option>             
<?php
...
<div style="display: inline" title="Remove contacts from the reminder list by double clicking the entry.">
        ...
        (Only Contacts with an E-Mail adr. will be sent a reminder)<br>
        ...
        <tr>
        <td colspan="2" align="center" valign="top">
        Send Reminder<br><input id="srval" name="nef[srval]" type="text" style="WIDTH: 30px" value="3">
...
?>       </td>
    ...
    <td width="70%" align="left" valign="top">
      <INPUT title="Check this box to create an Event Series" type="checkbox" id="eventrepeat" name="nef[eventrepeat]" LANGUAGE=javascript onclick="return eventrepeat_onclick()">
      <LABEL for="eventrepeat" title="Check this box to create an Event Series"><b>Event Series</b></LABEL>

<div id="divseriesevent" style="display: none">
      ...
      <table border="0" cellspacing="0" cellpadding="0" width="100%">
        ...
        <hr size="1">
        <input type="radio" value="1"   title="Select this for a Daily Event." name="nef[estype]" id="rbday" maxLength=0 LANGUAGE="javascript" disabled onclick="return rbday_onclick()">
        <label title="Select this for a Daily Event." for="rbday">Daily</label><br>
        <input type="radio" value="2"  title="Select this for a Weekly Event." name="nef[estype]" id="rbweek" maxLength=0 LANGUAGE="javascript" disabled onclick="return rbweek_onclick()">
        <label title="Select this for a Weekly Event." for="rbweek">Weekly</label><br>
        <input type="radio" value="3" title="Select this for a Monthly Event." name="nef[estype]" id="rbmonth" maxLength=0 LANGUAGE="javascript" disabled onclick="return rbmonth_onclick()">
        <label title="Select this for a Monthly Event." for="rbmonth">Monthly</label><br>
        <input type="radio" value="4"  title="Select this for a Yearly Event." name="nef[estype]" id="rbyear" maxLength=0 LANGUAGE="javascript" disabled onclick="return rbyear_onclick()">
        <label title="Select this for a Yearly Event." for="rbyear">Yearly</label>
            </td>
          <td width="85%" valign="top"><b>&nbsp;Occurance:</b>
        <hr size="1">
      <DIV title="Configure your Daily Event." style="DISPLAY: none" id="divday">
      
<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=0>
	<TR>
		<TD NOWRAP width="100%">
			<input title="Select this for an event that occurs every so many days." checked type="radio" name="nef[daytype]" value="1" id="eachday">
			<label title="Select this for an event that occurs every so many days."for="eachday">Every</label>&nbsp; 
            <input title="Enter the interval in days." type="text" id="eachdaycount" name="nef[eachdaycount]" value="1" size="5">&nbsp;Day(s)
		</TD>
	</TR>
	<TR>
		<TD NOWRAP width="100%">
			<input title="Select this for an event that occurs every weekday." type="radio" name="nef[daytype]" value="2" id="eachweekday">
      ...
      <DIV title="Configure your Weekly Event." style="DISPLAY: none" id="divweek">
      
<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=0>
	<TR>
		<TD NOWRAP colspan="4" width="100%">Every&nbsp; 
        <input title="Enter the weekly interval." type="text" id="eachweekcount" name="nef[eachweekcount]" value="1" size="5">&nbsp;Week(s)&nbsp;on
		</TD>
	</TR>
	<TR>
		<TD NOWRAP width="25%">
	...
	<TR>
      ...
      <DIV title="Configure your Monthly Event." style="DISPLAY: none" id="divmonth">
<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=0>
	<TR>
		<TD NOWRAP width="100%">
			<input title="Select this for an event that occurs on the same day of every so many months." checked type="radio" name="nef[dayofmonth]" value="1" id="dayofmonth1">
...
<?php
    ...
    ?>                
            ...
            &nbsp;Day,&nbsp;Every&nbsp;
            <input title="Enter the interval in months" type="text" id="dayofmonthcount" name="nef[dayofmonthcount]" value="1" size="5">&nbsp;Month(s)
		</TD>
	</TR>
	<TR>
		<TD NOWRAP width="100%">
			<input title="Select this for an event that occurs on a certain day of every so many months." type="radio" name="nef[dayofmonth]" value="2" id="dayofmonth2">
...
<?php
    ...
    ?>                    
            ...
            <input type="text" title="Enter the interval in months." id="whichdayofmonthcount" name="nef[whichdayofmonthcount]" value="1" size="5">&nbsp;Month(s)
		</TD>
	</TR>
</TABLE>

      </DIV>
      <DIV title="Configure your Yearly Event." style="DISPLAY: none" id="divyear">
<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=0>
	<TR>
		<TD NOWRAP width="100%">
			<input title="Select this for an event that occurs on the same day every year." type="radio" checked name="nef[dayofmonthyear]" value="1" id="dayofmonthyear1">
...
<?php
	...
	<TR>
		<TD NOWRAP width="100%">
			<input title="Select this for an event that occurs on a certain day of a certain month of every year." type="radio" name="nef[dayofmonthyear]" value="2" id="dayofmonthyear2">
...
<DIV style="DISPLAY: none" id="divsend">
        <hr size="1">
    ...
    <input title="Select this if the event has no end date." type="radio" name="nef[eventend]" checked value="0" id="eventend1" LANGUAGE=javascript onclick="return eventend1_onclick()">
    <label title="Select this if the event has no end date." for="eventend1">Never</label></td>
              </tr>
              <tr>
                <td width="15%">&nbsp;</td>
                <td width="85%" colspan="2">
                <input title="Select this if the event ends after so many occurances." type="radio" name="nef[eventend]" value="1" id="eventend2" LANGUAGE=javascript onclick="return eventend2_onclick()">
                <label title="Select this if the event ends after so many occurances." for="eventend2">End After</label>&nbsp;
                <input title="Enter the number of events to add." type="text" id="eventendafter" name="nef[eventendafter]" value="10" size="5">&nbsp;occurances</td>
              </tr>
              <tr>
                <td width="15%">&nbsp;</td>
                <td width="15%">
                <input title="Select this if the event ends after a certain date." type="radio" name="nef[eventend]" value="2" id="eventend3" LANGUAGE=javascript onclick="return eventend3_onclick()">
...
<div style="display:inline" title="Select the year after which the event ends" >
                    ...
                    <tr>
                      <td width="20%">
<div style="display:inline" title="Select the day after which the event ends" >
                      <select disabled size="1" id="eventendday" style="WIDTH: 47px" name="nef[eventendday]" LANGUAGE=javascript onchange="return eventendday_onchange()">
<?php
    for($lc=1;$lc<=31;$lc++) {
        if($lc < 10) {$mlc = "0".$lc;} else {$mlc=$lc;}
		echo "        <option ";
    ...
    ?>                       
                      ...
                      <td width="20%">
<div style="display:inline" title="Select the month after which the event ends" >
                      <select disabled size="1" id="eventendmonth" style="WIDTH: 65px" name="nef[eventendmonth]" LANGUAGE=javascript onchange="return eventendmonth_onchange()">
    <?php
    for($lc=1;$lc<=12;$lc++) {
        if($lc < 10) {$mlc = "0".$lc;} else {$mlc=$lc;}
		echo "        <option ";
    ...
    ?>                       
                      ...
                      <td width="20%">
<div style="display:inline" title="Select the year after which the event ends" >
                      <select disabled size="1" id="eventendyear" style="WIDTH: 70px" name="nef[eventendyear]" LANGUAGE=javascript onchange="return eventendyear_onchange()">
    <?php
    for($lc=($fyear-1);$lc<=($fyear+4);$lc++) {
		echo "        <option ";
        if($zxfdate["year"]==$lc) {
