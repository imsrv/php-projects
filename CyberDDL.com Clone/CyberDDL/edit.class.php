...
class edit extends config {
    function location() {
	header("Location:http://www.cyberddl.info/reqtot.php");
}
	function headers() {
...
&nbsp;&nbsp; - Press Only Once!! Submission May Take A While</td></tr></form></table>";
	...
	function main($user) {
		...
		If you have any difficulties or questions don't hesitate to contact me at : emad_fankoosh@hotmail.com</a>.<br><br>";
...
<br><b>$t[req]</b> requests have been added<br>
...
Most Popular download is <b><a href=\"$p[url]\" target=\"_blank\">$p[name]</a></b> (<a href=\"$p[surl]\" target=\"_blank\">$p[sname]</a>) with <b>$p[v]</b> downloads";
	...
	function stats($id = "") {
		global $page, $del, $ed, $list, $do;
		if ($do == "del") {
			echo "<b>".count($list)."</b> links removed<br><br>";
...
<form name=\"del\" action=\"admin.php?go=stats&do=del&page=$page\" method=\"POST\">
		<table border=\"0\" cellspacing=\"1\" cellpadding=\"3\" bgcolor=\"#212C3B\"><tr style=\"color:#FFFFFF;\" align=\"center\">
		<td>Edit/Del</td><td>Type</td><td>Title</td><td>Date</td><td>Submitter</td><td>Views</td></tr>";
			while ($row = mysql_fetch_array($get)) {
	echo "<tr bgcolor=\"#FFFFFF\"><td><a href=\"admin.php?go=stats&id=$row[id]\">Edit</a> <input type=\"checkbox\" name=\"list[]\" value=\"$row[id]\"></td><td align=\"center\">$row[type]</td><td>
...
<td><a href=\"$row[surl]\" target=\"_blank\">$row[sname]</a></td><td align=\"center\">$row[views]</td></tr>\n";
			...
			echo "</table><input type=\"submit\" name=\"submit\" value=\"Del Selected\" class=\"form\"></form><br><br>";
...
<tr><td></td><td><input type=\"Submit\" value=\"Update!\" class=\"form\"></td></tr></table>";
	...
	function add($location, $insert) {
		global $HTTP_POST_VARS, $title, $url, $sname, $surl, $email;
		if (!$HTTP_POST_VARS)
			$this->form($location);
		elseif (!$title || !$url || !$sname || !$surl)
	...
	function req($go) {
		...
		if ($go == "Insert") {
			...
			if ($antallet > $this->max_dl && $this->max_dl > 0) {
				...
				$max_get = mysql_query("SELECT * FROM $tabellen ORDER BY id LIMIT 0,$max_limit");
			...
			$get = mysql_query("SELECT * FROM $this->mysql_tb_req WHERE $hvahvor");
			mysql_query("DELETE FROM $this->mysql_tb_req WHERE $hvahvor");
			echo "<b>".count($list)." requests inserted to database!</b>";
			while ($row = mysql_fetch_array($get)) {
				mysql_query("INSERT INTO $this->mysql_tb_req2 (type, req, sname, date, email)
				VALUES ('$row[type]','$row[req]','$row[sname]','$dato','$row[email]')");
			}
			...
			$hvahvor = $this->run($list);
			mysql_query("DELETE FROM $this->mysql_tb_req WHERE $hvahvor");
			echo "<b>".count($list)." requests in que deleted!</b><br><br><a href=\"javascript:history.go(-1)\">back</a>";

		} else {
			$get = mysql_query("SELECT * FROM $this->mysql_tb_req");
			echo "<form name=\"f1\" action=\"admin.php?go=req\" method=\"POST\">".mysql_num_rows($get)." requests submitted for review<br><br>\n\n";
			if (mysql_num_rows($get)) {
				while ($row = mysql_fetch_array($get)) {
					if (!$row[email])
						$row[email] = "no email";
					echo "<input type=\"checkbox\" name=\"list[]\" value=\"".$row[id]."\" class=\"box\">".$row[req]."<a> - </a>".$row[type]."<a> - </a>".$row[sname]."</a>- $row[email])<br>\n";
				}
		...
		if ($go == "Insert") {
			...
			if (mysql_num_rows($get)) {
				while ($row = mysql_fetch_array($get)) {
					if (!$row[email])
						$row[email] = "no email";
					echo "<input type=\"checkbox\" name=\"list[]\" value=\"".$row[id]."\" class=\"box\"><a href=\"".$row[url]."\" target=\"_blank\">".$row[title]."</a> (<a href=\"$row[surl]\" target=\"_blank\">$row[sname]</a> - $row[email])<br>\n";
				}
				echo "<br><input type=\"Submit\" name=\"sub\" value=\"Insert\" class=\"form\">
				<input type=\"Submit\" name=\"sub\" value=\"Delete\" class=\"form\"></form>";
			}
    ...
    function reqtot($go) {
	echo "<b><a href=\"http://free.deluxnetwork.com/~emad/reqtot.php\">Apply Requested Applications To The Database</a>.<br><br>";
	}
...
<a href=\"admin.php?go=stats&id=$row[id]\">Edit</a> | <a href=\"".$row[url]."\" target=\"_blank\">".$row[title]."</a> ($row[reports] ";
	...
	function edit_blacklist() {
		...
		<br>One url per line<br><b>Remeber to enter url with trailing slash at the end (example: http://www.cyberddl.info/)</b></form>\n";
	}
}
?>