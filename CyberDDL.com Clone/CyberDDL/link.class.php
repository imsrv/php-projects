...
<td><a href=\"$row[url]\" target=\"_blank\">$row[url]<a></td><td>$row[inn]</td><td>$row[ut]</td><td>".@round($row[inn]/$row[ut], 2)."</td></tr>\n";
	...
	function add() {
		...
		elseif ($ll && (!$ll[user] || !$ll[pass] || !$ll[url] || !$ll[email])) {
			...
			http://www.cyberddl.info/in.php?id=$ll[user]<br><br><b>This Information has been emailed you!</b><br><br>
...
<tr><td></td><td><input type=\"Submit\" value=\"Update!\" class=\"form\"></td></tr></form></table>";
	...
	function get($number="5", $max="20", $start="0") {
		...
		while($row = mysql_fetch_array($get)) {
			if ($ii == $number-1)
				echo "</td>";
			if ($i != $max-$number && $ii == $cols-1) {
				echo "<td>";
				$ii=0;
			} else
				$ii=0;
echo "<a href=\"ut.php?id=$row[id]\" target=\"_blank\" title=\"In:$row[inn]\" onmouseover=\"window.status='In: $row[inn]'; return true;\" onmouseout=\"window.status=''\">$i. $row[title]</a><br>";
			$i++;
			$ii++;
		}
		if ($max > $i) {
			while ($max != $i) {
				if ($ii == $number-1)
					echo "</td>";
				if ($ii != $max-1 && $ii == $number-1) {
					echo "<td>";
					$ii=0;
				} elseif ($ii == $number-1)
					$ii=0;

				echo "<a href=\"join.php\" target=\"mainFrame\" title=\"Join Le\">Your site here?</a><br>\n";
				$i++;
				$ii++;
			}
		}
