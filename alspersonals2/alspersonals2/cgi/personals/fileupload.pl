...
BEGIN {
		...
		# Allowing a user to upload a file named index can be quite problematic.
		# It is best just to leave the value of files_named_index at 0
...
###########################################
	...
	if ( (!(-e $userimagesdir)) ||
	 ...
	 (!(-d $userimagesdir)) ) {
		...
		<BODY BGCOLOR=\"#FFFFFF\">$mainheader $menu1$menu2
...
##############################################################################
	...
	if ($TOTAL_BYTES > $maxfilesize && $maxfilesize > 0) {
		...
		$mainheader $menu1 $menu2 <br><br>
		$fupl7
		<center><b>$mxname: $maxfilesize</b></center>
		$botcode
		\n";
		exit;
		}
