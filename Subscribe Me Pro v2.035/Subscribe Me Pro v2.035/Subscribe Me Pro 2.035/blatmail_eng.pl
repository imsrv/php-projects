...
open (SUBJECT2, "<$memberinfo/mmailsubject.txt");
if ($LOCK_EX){ 
      flock(SUBJECT2, $LOCK_EX); #Locks the file
	}

@database_subject = <SUBJECT2>;
close (SUBJECT2);


foreach $linessub(@database_subject) {
          @edit_subject = split(/\|/,$linessub);
chomp($linessbu);
}

$subject = "$edit_subject[0]";
$from = "$edit_subject[2]";
chomp($edit_subject[1]);

$servername = $edit_subject[3];
chomp($servername);

$cgiurl = $edit_subject[4];
chomp($cgiurl);

$time = $edit_subject[6];
chomp($time);

$tracking = $edit_subject[7];
chomp($tracking);

$useurl = $edit_subject[8];
chomp($useurl);

$usehtml = $edit_subject[9];
chomp($usehtml);
...
open (FILE,"<$lists/$edit_subject[1].signature"); #### Full path name from root.
@closing  = <FILE>;
close(FILE);


open (LIST,"<$lists/$edit_subject[1].list");
 if ($LOCK_EX){ 
...
open (LOG, ">$maildir/$time.txt");


$user_count=0;
foreach $linesa(@database_array) {
chomp($linesa);
$linesa =~ s/\r//;
...
open (MESSAGE2, "<$memberinfo/$time-mm.txt");
if ($LOCK_EX){ 
      flock(MESSAGE2, $LOCK_EX); #Locks the file
	}

@database_message2 = <MESSAGE2>;
close (MESSAGE2);

foreach $mlines(@database_message2) {


if (lc $tracking eq 'on') {

     $mlines =~ s/page=/http:\/\/$servername$cgiurl?which=$time&page=/;
...
# All done
			...
			$body .= "<A HREF=\"http://$servername$cgiurl?r=1&l=$edit_subject[1]&e=$linesa\"><FONT SIZE=\"-1\" FACE=\"arial,  helvetica\"><B>Unsubscribe $linesa</B></FONT></A><BR>";
		        $body .= "<BR><BR></P>";
                  unless ($edit_subject[10]) {
			...
			$body .= "http://$servername$cgiurl?r=1&l=$edit_subject[1]&e=$linesa\n";
                        $body .= "\n\n";
                  unless ($edit_subject[10]) {
...
&sendmail("$linesa","$from","$subject","$body");
...
$user_count++;
        ...
        unlink("$memberinfo\\mmailsubject.txt");
        unlink("$memberinfo\\$time-mm.txt");
...
$body = "";
			...
			$body .=  "Subject: Subscribe Me Mass Mailing Status!\r\n\r\n";
...
&sendmail("$from","$from","Subscribe Me Mass Mailing Status!","$body");
}
