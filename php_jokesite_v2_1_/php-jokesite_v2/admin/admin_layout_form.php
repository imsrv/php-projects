...
if ($HTTP_POST_VARS['todo']=="save") {
	...
	$fp=fopen(DIR_SERVER_ROOT."site_design_configuration.php","w");
    fwrite($fp,"<?\n");
...
//-->
	...
	<? refresh(HTTP_SERVER_ADMIN.$this_file_name);exit;?>
 <table width="<?=BIG_TABLE_WIDTH?>" cellspacing="0" cellpadding="1" border="0" align="center">
 <tr>
     <td bgcolor="<?=TABLE_HEAD_BGCOLOR?>" align="center"><font face="Verdana, Arial" size="2" color="white"><b>Layout manager</b></font></td>
 </tr>
 <tr>
   <td bgcolor="<?=TABLE_BORDERCOLOR?>">
     <TABLE border="0" cellpadding="4" cellspacing="0" bgcolor="<?=INSIDE_TABLE_BG_COLOR?>" width="100%">
            <tr>
                <td align="center"><font face="helvetica, verdana, arial" size="" color="<?=TEXT_FONT_COLOR?>"><b>Successfull update.</b></font></td>
            </tr>
            <tr>
...
elseif ($HTTP_POST_VARS['todo']=="preview") {
   ...
   while (list($header, $value) = each($HTTP_POST_VARS)) {
           echo "define('".$header."','".$value."');\n";
     }//end while (list($header, $value) = each($HTTP_POST_VARS))
}
else {
$fp=fopen(DIR_SERVER_ROOT."site_design_configuration.php","r");
?>
<form method="post" action="<?=HTTP_SERVER_ADMIN.$this_file_name?>" name="layout" onSubmit="return saviee();">
<input type="hidden" name="todo" value="save">
<table cellspacing="0" cellpadding="1" border="0" width="<?=BIG_TABLE_WIDTH?>" align="center">
<tr>
	<td bgcolor="<?=TABLE_HEAD_BGCOLOR?>" align="center"><font face="Verdana, Arial" size="2" color="white"><b>Layout manager</b></font></td>
</tr>
<tr>
	<td bgcolor="<?=TABLE_BORDERCOLOR?>">
		<TABLE border="0" cellpadding="4" cellspacing="0" bgcolor="<?=INSIDE_TABLE_BG_COLOR?>" width="100%" class="reg">
<?
$i=1;
$comment = false;
$commenttext = '';
...
while (!feof($fp)) {
   ...
   if (!empty($str) && ($str != "\r\n") && ($str != "\n") && ($str != "\r")) {
	    if ($usercomment) {
					...
					echo "<tr><td colspan=\"3\" align=\"center\"><font size=\"2\" color=\"#FF0000\"><b>".ucfirst(eregi_replace("^##","",$usercommenttext))."</b></font></td></tr>";
		} 
	    if ($scriptcomment) {
					$scriptcomment = false;
		} 
        if (eregi("^define\(['](.*)['|.?],['|.?| ](.*)\)",htmlspecialchars($str),$regexp)) {
			...
			if ($comment && !$usercomment) {
					$comment = false;
					//echo "<td>".$commenttext."</td>";
					echo "<td align=\"right\">".$commenttext." = </td>";
			}
			else {
				   echo "<td align=\"right\">".$regexp[1]." = </td>";	
			}
			if (eregi("#.*",eregi_replace("'","",$regexp[2]),$tregexp)) {
                echo "<td><input type=\"text\" name=\"".$regexp[1]."\" value=\"".eregi_replace("'","",$regexp[2])."\"></td>";
                echo "<td><table id=\"id".$i."\" bgcolor=\"".$tregexp[0]."\" border=\"1\">";
                echo "<tr><td><span onclick=\"PcjsColorChooser(document.forms.layout.".$regexp[1].",'value','id".$i."', event)\" style=\"cursor: hand\">&nbsp;&nbsp;&nbsp;&nbsp;</span></td></tr>";
                echo "</table></td>";
                $i++;
            }
            else {
                echo "<td colspan=\"3\"><input type=\"text\" name=\"".$regexp[1]."\" value=\"".eregi_replace("'","",$regexp[2])."\"></td>";
            }
			echo "<input type=\"hidden\" name=\"inputs[]\" value=\"".$regexp[1]."\">";
                echo "</tr>";
        }
		else {
		  if (eregi("^//(.*)", htmlspecialchars($str), $regexpcomment)) {
				//echo "<tr><td colspan=3>".$regexpcomment[1]."</td></tr>";
				echo "<input type=\"hidden\" name=\"inputs[]\" value=\"//".$regexpcomment[1]."\">";
				if ($regexpcomment[1][0]=="#") {
					$usercomment = true;
					$usercommenttext =  $regexpcomment[1];
				}
				else {
					$comment = true;
					$commenttext =  $regexpcomment[1];
				}
			}
			else if (eregi("^#(.*)", htmlspecialchars($str), $regexpcomment)) {
				//echo "<tr><td colspan=3>".$regexpcomment[1]."</td></tr>";
				echo "<input type=\"hidden\" name=\"inputs[]\" value=\"#".$regexpcomment[1]."\">";
				$comment = true;
				$commenttext =  $regexpcomment[1];
				...
				echo "<input type=\"hidden\" name=\"inputs[]\" value=\"/*".$regexpcomment[1]."\">";
				$scriptcomment = true;
				$scriptcommenttext =  $regexpcomment[1];
			}
			else if ($str == "<?" || $str == "?>") {
			}
			else {
...
?>
<tr>
        <td align="left"><input type="button" name="preview" value="Preview" onClick="prew();" class="button"></td>
        <td colspan="2" align="right"><input type="submit" name="save" value="Save" class="button"></td>
</tr>
</table>
</td></tr></table>
</form>
...
<tr><td style="border: 1px solid #848484"><form name="pcjsform" action="<?=HTTP_SERVER_ADMIN.$this_file_name?>" method="post">
<table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#D6D6CE" style="border: 1px solid #FFFFFF">
       <tr>
                ...
                <td height="25" valign="middle">
                        <div align="right" style="background:#0000CC; height: 20px;"><span style="position: relative; font-size: 14px; background: #DDDDDD; width: 15px; height: 15px; top: 1px; vertical-align: middle; text-align: center; border: 1px solid #000000"><a href="javascript:closs();" onClick='PcjsInternalClosePopup()' class="closebutton">&nbsp;x&nbsp;</a></span></div>
                </td>
                <td height="3" width="2"><img src="images/pix-t.gif" width="2" height="3"></td>
        </tr>
        <tr>
                  ...
                  <td bgcolor="#FFFFFF" onClick="PcjsInternalSelectColor('#FFFFFF')" width=4 height="8"><img src="images/pix-t.gif" width="4" height="8"></td>
                                                ...
                                                <td colspan="18" align="center">
                                                                <table border="0" cellpadding="0" cellspacing="0">
                                                                        <tr>
 <td width="50%">            <font face='Verdana, Arial, Helvetica, sans-serif' size=2 color=#0000CC>Color : </font> <input type=text name="color" size="10" maxlength="8" onChange='makeChanges(0);' style="font-size:8pt;"></td> <td align="center" valign="middle"><table border="0" cellspacing="0" cellpadding="0" align="center" width="100%"> <tr width="30" height="35"> <td id="pcjscell" width="30" height="35" align="center" valign="top"><img src="images/pix-t.gif" width="20" height="25"></td>   </tr>   </table>
                                            </td>
      </tr>
                                                                </table>
                                                        </td>
