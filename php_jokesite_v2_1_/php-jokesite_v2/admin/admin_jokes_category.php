...
$page_title = "Joke Category (".$language.")";
$nr_of_cols = 4;
...
$edit_field_value = $HTTP_GET_VARS[$edit_field_name];
	...
	$display_nr = 10;
isset($HTTP_GET_VARS['order_by']) ? ($order_by = $HTTP_GET_VARS['order_by']).($order_type = $HTTP_GET_VARS['order_type']) : ($order_by = $edit_field_name).($order_type = "asc");
...
if ($HTTP_POST_VARS['delete_button'] == "Delete Checked" || $HTTP_GET_VARS['delete_this'])	//delete via ID
{
	...
	for( $i = 0 ; $i < sizeof($HTTP_POST_VARS[$primary_id_name]); $i++ )
	{
		...
		{
			$delete_SQL = "delete from $database_table_name where $primary_id_name='".$HTTP_POST_VARS[$primary_id_name][$i]."'<br>";
...
$link_get_vars = $get_vars."&from=".$from;

$condition = " order by ".$order_by." ".$order_type;
...
<tr>
	<td bgcolor="<?=TABLE_BORDERCOLOR?>" align="center"> 
		...
		<tr bgcolor="<?=TABLE_HEAD_BGCOLOR?>">
			...
			<td align="center" width="68%"><font color="<?=TABLE_HEAD_FONT_COLOR?>"><b><a href="<?=$this_file_name.$title_get_vars?>&order_by=<?=$edit_field_name?>&order_type=<?=$HTTP_GET_VARS['order_by'] == $edit_field_name ? ($HTTP_GET_VARS['order_type'] == "asc" ? "desc" : "asc") :  "asc"; ?>" style="color:#ffffff">Categories</a></b></font></td>
...
?>
		<tr bgcolor="<?=INSIDE_TABLE_BG_COLOR?>">
			...
			<td class="text" align="center">
				...
				&nbsp;&nbsp;&nbsp;&nbsp;[ 
				<a onClick="value=forms['<?=$edit_field_name?>[]'];this.href += escape(value[<?=$i?>].value);" href="<?=$this_file_name?>?<?=$link_get_vars?>&<?=$primary_id_name?>=<?=$result_array[$i][$primary_id_name];?>&<?=$edit_field_name?>=">Update</a>
...
?>
		...
		<tr bgcolor="<?=INSIDE_TABLE_BG_COLOR?>">
			<td colspan="<?=$nr_of_cols?>">
				<table align="center" border="0" cellspacing="0" cellpadding="0" width="100%">
				<tr>
					<form method=post action="<?=$this_file_name?>?display_nr=<?=$display_nr?>&order_by=<?=$primary_id_name?>&order_type=asc" name="add_new_form">
					<td width="14%">
