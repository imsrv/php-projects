...
function check_form() {
        var error = 0;
        var error_message = "<?=eregi_replace("\"","\\\"",JS_ERROR);?>";

        var lng = document.addlng.lng.value;

        //Validation for LANGUAGE_NAME
        if (lng == "") {
                error_message = error_message + "<?=eregi_replace("\"","\\\"",LANGUAGE_NAME_ERROR);?>\n";
                error = 1;
        }
        ...
        if (!exist) {
                error_message = error_message + "<?=eregi_replace("\"","\\\"",BASE_LANGUAGE_ERROR);?>\n";
                error = 1;
        }
...
function check_form_dellng() {
        ...
        if (!exist) {
                error_message = error_message + "<?=eregi_replace("\"","\\\"",DELETE_LANGUAGE_ERROR);?>\n";
                error = 1;
        }
        ...
        if (error == 1) {
                ...
                return confirm("<?=eregi_replace("\"","\\\"",TEXT_CONFIRM_LANGUAGE_DELETE)?>");
        }
}
...
function check_form_editlng() {
        ...
        if (!exist) {
                error_message = error_message + "<?=eregi_replace("\"","\\\"",EDIT_LANGUAGE_ERROR);?>\n";
                error = 1;
        }
