...
#Eg. http://www.yourserver.com/cgi-bin/instachat20.cgi
$cgi_url = "http://!CGIURL!/instachat20.cgi";

#Temporary directory to store messages and user files CHMOD to 777
#Eg. /path/to/save/instachat20/file/
$tmp_dir = "!SAVEDIR!/";
...
#0 = Display as scrolling box / 1 = Display as pop up window
$mode = 1;
...
#################### MODE 1 #############################
#Size of pop up chat box (SHIFT-C to pop up)
$pop_width = 450;
$pop_height = 400;
...
print <<END
<HTML>
<TITLE>InstaChat 2.0</TITLE>

<FRAMESET ROWS=*,60 FRAMEBORDER=1 FRAMESPACING=0 BORDER=0>
<FRAME NAME="setframes" SRC="$cgi_url?area=start&url=1&username=$username">
<FRAME NAME="setframespass" SRC="$cgi_url?area=startpass&username=$username">
</FRAMESET>
...
document.writeln('<TR><TD><CENTER><B>InstaChat 2.0 (You are $username)</B></CENTER></TD></TR>');
...
function reload_Page()
 {
 parent.setframes.location.href = '$cgi_url?area=start&url=1&username=$username';
...
<BODY BGCOLOR="$background">
<FORM METHOD=POST NAME="chatform" ACTION="$cgi_url" target="setframes" onSubmit="return submit_form();">
...
document.onkeypress = function (evt) {
  ...
  if (r == 'SHIFT67')
   {
   var IChatWindow;
   window.open('$cgi_url?area=1&url=1',IChatWindow++,'width=$pop_width,height=$pop_height');
   }
