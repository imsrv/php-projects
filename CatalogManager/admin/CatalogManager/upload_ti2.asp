...
If (CStr(UploadFormRequest("MM_update")) <> "" And CStr(UploadFormRequest("MM_recordId")) <> "") Then
  ...
  MM_columnsStr = "ImageFileThumb2|',none,''"
...
If (Request.QueryString("ItemID")  <> "") Then 
  itemdetail__MMColParam = Request.QueryString("ItemID") 
End If
%>
<%
set itemdetail = Server.CreateObject("ADODB.Recordset")
itemdetail.ActiveConnection = MM_catalogmanager_STRING
itemdetail.Source = "SELECT *  FROM tblCatalog  WHERE ItemID = " + Replace(itemdetail__MMColParam, "'", "''") + ""
...
<form ACTION="<%=MM_editAction%>" METHOD="post" name="form1" enctype="multipart/form-data" onSubmit="checkFileUpload(this,'GIF,JPG,JPEG,BMP,PNG');checkFileUpload(this,'GIF,JPG,JPEG,BMP,PNG',true,'','','','','','','');return document.MM_returnValue">
  ...
  <input type="hidden" name="MM_recordId" value="<%= itemdetail.Fields.Item("ItemID").Value %>">
</form>
<p>&nbsp;</p>
</body>
</html>
