...
If (CStr(Request("MM_update")) = "form1" And CStr(Request("MM_recordId")) <> "") Then
  ...
  MM_columnsStr = "CategoryName|',none,''|CategoryDesc|',none,''|CategoryImageFile|',none,''"
...
Category.Source = "SELECT *  FROM tblCatalogCategory  WHERE CategoryID = " + Replace(Category__MMColParam, "'", "''") + ""
...
<form ACTION="<%=MM_editAction%>" METHOD="POST" name="form1" >
  ...
  <tr>
    ...
    <td width="86%"><input name="GPCName" type="text" id="GPCName2" value="<%=(Category.Fields.Item("CategoryName").Value)%>" size="50"></td>
  </tr>
  <tr>
    <td height="26" class="tableheader">Category Desc</td>
    <td><textarea name="GPCDesc" cols="50" id="GPCDesc2"><%=(Category.Fields.Item("CategoryDesc").Value)%></textarea>
</td>
  </tr>
    <tr>
      <td  class="tableheader"><p>Category Image</p>
      <p> <a href="javascript:;" onClick="MM_openBrWindow('update_image_category.asp?cid=<%=(Category.Fields.Item("CategoryID").Value)%>','image','width=300,height=150')">Click
      here to Update Image</a> </p></td>
      <td valign="baseline"> 
	  <% if Category.Fields.Item("CategoryImageFile").Value <>"" Then %>
	  <img src="../../../applications/CatalogManager/images/<%=(Category.Fields.Item("CategoryImageFile").Value)%>"><br>
	  <%end if%>
      <input name="ImageFile" type="text" id="ImageFile" value="<%=(Category.Fields.Item("CategoryImageFile").Value)%>">
</td>
    </tr>
    <tr>
