...
%>
			    ...
			    <font size="1"><%=(GPC.Fields.Item("GPCName").Value)%></font></a>	  
			    <br>
                <% 
...
%>
 ...
 <% If Not Category.EOF Or Not Category.BOF Then %>
        <table width="100%" border="0" cellpadding="5" cellspacing="0" class="tableborder">
          <tr>
            <td><strong><font size="1">Category 
            <% if Request.QueryString("gpcid") <> "" then %>  
            of 
		       
            <%=(Category.Fields.Item("GPCName").Value)%>
            <% end if%>
            </font></strong><font size="1">&nbsp;	</font> </td>
          </tr>
          <tr>
            <td valign="top">
              <div align="left">
...
%>
	            ...
	            <font size="1"><%=(Category.Fields.Item("CategoryName").Value)%></font></a>
                 <br>
                <% 
...
%>
        ...
        <% End If ' end Not Category.EOF Or NOT Category.BOF %>
          ...
          <tr>
            <td><strong><font size="1">Sub Category of <%=(SubCategory.Fields.Item("CategoryName").Value)%></font></strong></td>
          </tr>
          <tr>
            <td valign="top">
              <% 
...
%>
                <a href="<%=request.servervariables("URL")%>?scid=<%=(SubCategory.Fields.Item("SubCategoryID").Value)%><% If Request.QueryString("gpcid") <> "" Then%>&gpcid=<%=request.querystring("gpcid")%><%end if%><% If Request.QueryString("cid") <> "" Then%>&cid=<%=request.querystring("cid")%><%end if%><%If Request.QueryString("mid") <> "" Then %>&mid=<%=request.querystring("mid")%><%end if%><%If Request.QueryString ("mid2")<> "" Then %>&mid2=<%=request.querystring("mid2")%><%end if%><%If Request.QueryString ("mid3")<> "" Then %>&mid3=<%=request.querystring("mid3")%><%end if%><%If Request.QueryString ("incid")<> "" Then %>&incid=<%=request.querystring("incid")%><%end if%>">	
			    <font size="1"><%=(SubCategory.Fields.Item("SubCategoryName").Value)%></font></a><br>
              <% 
