...
///////////////////////////////////////////////////////////////////////////////
	...
	*/
	...
	if($isSetup == 'no')
		header("Location: $admindir/setup.php");
	...
	function ShowRecentArticles($NumArticles = 10)
	{
		...
		?>
            <tr>
            <td width="<?php if($showAuthorImages) echo '3%'; else echo '0%'; ?>">
				<?php if($showAuthorImages) { ?><img src="imageview.php?what=getAuthorPic&authorId=<?php echo $aRow["aAuthorId"]; ?>">&nbsp;<?php } ?>
            </td>
            <td width="<?php if($showAuthorImages) echo '97%%'; else echo '100%'; ?>" valign="top">
	...
	{
		global $isVBulletinForum, $isPHPBBForum, $forumDBServer, $forumDBUser, $forumDBPass, $forumDBName, $forumDBPath, $forumDBPrefix;
		...
		if($isVBulletinForum == true || $isPHPBBForum == true)
		{
			...
			if($forumDBPrefix != "[None]" || $forumDBPrefix != "[None]")
				$prefix = $forumDBPrefix;
			else
				$prefix = '';
			...
			if($isVBulletinForum == true)
				...
				$vbQuery = @mysql_query("select topic_id, topic_title from " . $prefix . "topics order by topic_id desc limit $NumPosts");
			...
			if($isVBulletinForum == true)
			{
				...
				{
				?>
					<a href="<?php echo $forumDBPath; ?>/showthread.php?threadid=<?php echo $frow[0]; ?>"><span class="Link5"><?php echo $frow[1]; ?></span></a><br><img src="blank.gif" width="1" height="10"><br>
				<?php
				}
			}
			else if($isPHPBBForum == true)
			{
				...
				{
				?>
					<a href="<?php echo $forumDBPath; ?>/viewtopic.php?t=<?php echo $frow[0]; ?>"><span class="Link5"><?php echo $frow[1]; ?></span></a><br><img src="blank.gif" width="1" height="10"><br>
				<?php
				}
			}
			echo "</td></tr>";
	...
	function ShowComments($id = 0, $type = 'article')
		{

			//Shows comments on articles if its turned on in the config menu

			global $siteComments, $siteCommentsShow;
			...
			if($siteComments)
				{
						...
						$query = mysql_query("SELECT * FROM tbl_Comments WHERE cType = '$type' and cId = '$id' and cVisible = '1' ORDER BY pk_cId DESC");
						$query2 = mysql_query("SELECT * FROM tbl_Comments WHERE cType = '$type' and cId = '$id' and cVisible = '1' ORDER BY pk_cId DESC LIMIT $siteCommentsShow");


					?>

						<br>
						<span class="BodyHeading">&nbsp;&nbsp;Article Comments</span>&nbsp;&nbsp;
						<a href="comment.php?type=article&id=<?php echo $_GET['articleId']; ?>"><img align="absbottom" src="images/plus.gif" border="0"><strong>Add Comment</strong></a> | 
						<a href="articles.php?articleId=<?php echo $_GET['articleId']; ?>&page=<?php echo $_GET['page']; ?>&show=all"><img align="absbottom" src="images/folder2.gif" border="0"><strong>View All</strong></a> <span class='Text1'>(<?php echo @mysql_num_rows($query); ?>)</span>
						<hr size="1" color="#CCCCCC" noshade width="96%">
									...
									?>
											...
											<tr>
												<td>
													...
													<span class="BodyHeading2">Poster:</span> <span class="BodyHeading3"><?php echo $row['cName']; ?></span><br>
													<span class="BodyHeading2">Added:</span> <span class="BodyHeading3"><?php echo MakeDate($row['cDateCreate']); ?></span><br><br>
													<span class='Text1'><?php echo nl2br(htmlspecialchars($row['cComment'])); ?></span>
