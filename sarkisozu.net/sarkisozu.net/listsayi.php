...
mysql_select_db($database_lyrics, $lyrics);
$query_sarkicilar = sprintf("SELECT id, sanatci, sarki FROM sarki WHERE sanatci <= '%s' ORDER BY sanatci ASC, sarki ASC", $colname_sarkicilar);
...
<br>
      ...
      <?php } // Show if recordset not empty ?> <br><center>
        ...
        <a href="<?php printf("%s?pageNum_sarkicilar=%d%s", $currentPage, 0, $queryString_sarkicilar); ?>"><<-Ýlk Sayfa</a>
        <?php } // Show if not first page ?>
        &nbsp;
        <?php if ($pageNum_sarkicilar > 0) { // Show if not first page ?>
        <a href="<?php printf("%s?pageNum_sarkicilar=%d%s", $currentPage, max(0, $pageNum_sarkicilar - 1), $queryString_sarkicilar); ?>"><-Önceki Sayfa</a>
        <?php } // Show if not first page ?>
        &nbsp;
        <?php if ($pageNum_sarkicilar < $totalPages_sarkicilar) { // Show if not last page ?>
        <a href="<?php printf("%s?pageNum_sarkicilar=%d%s", $currentPage, min($totalPages_sarkicilar, $pageNum_sarkicilar + 1), $queryString_sarkicilar); ?>">Sonraki Sayfa-></a>
        <?php } // Show if not last page ?>
        &nbsp;
        <?php if ($pageNum_sarkicilar < $totalPages_sarkicilar) { // Show if not last page ?>
        <a href="<?php printf("%s?pageNum_sarkicilar=%d%s", $currentPage, $totalPages_sarkicilar, $queryString_sarkicilar); ?>">Son Sayfa->></a>
        <?php } // Show if not last page ?>
