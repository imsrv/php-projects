...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "destekleyonet")) {
  $updateSQL = sprintf("UPDATE destek SET destek=%s WHERE id=%s",
                       GetSQLValueString($_POST['destek'], "text"),
                       GetSQLValueString($_POST['id'], "int"));

  mysql_select_db($database_lyrics, $lyrics);
  $Result1 = mysql_query($updateSQL, $lyrics) or die(mysql_error());

  $updateGoTo = "destekleupdate.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_lyrics, $lyrics);
$query_destekal = "SELECT * FROM destek";
$destekal = mysql_query($query_destekal, $lyrics) or die(mysql_error());
$row_destekal = mysql_fetch_assoc($destekal);
$totalRows_destekal = mysql_num_rows($destekal);
?>
...
<table height="520">
  ...
  <tr>
    ...
    <td width="60%" rowspan="2"><a href="../index.php">Anasayfa</a> | <a href="yonet.php">Admin Paneli</a> | <a href="destekleyonet.php">Destekleyenleri Yönet</a><br>
        <br>
        <center>
          <b>Destekleyenleri Yönet</b>
        </center><br>
      <center><table><tr><td><form action="<?php echo $editFormAction; ?>" method="POST" name="destekleyonet">
      <input type="hidden" name="id" value="<?php echo $row_destekal['id']; ?>"><center>
        <textarea name="destek" rows="20" cols="50"><?php echo $row_destekal['destek']; ?></textarea><br><input type="submit" value="güncelle"></center>
      <input type="hidden" name="MM_update" value="destekleyonet">
