...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "adminekle")) {
  $insertSQL = sprintf("INSERT INTO admin (`user`, pass, yetki) VALUES (%s, %s, %s)",
                       GetSQLValueString($_POST['user'], "text"),
                       GetSQLValueString($pass, "text"),
                       GetSQLValueString($_POST['yetki'], "text"));

  mysql_select_db($database_lyrics, $lyrics);
  $Result1 = mysql_query($insertSQL, $lyrics) or die(mysql_error());

  $insertGoTo = "moderatorekle_ok.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_lyrics, $lyrics);
$query_adminler = "SELECT * FROM `admin` WHERE yetki = '5' ORDER BY `user` ASC";
$adminler = mysql_query($query_adminler, $lyrics) or die(mysql_error());
$row_adminler = mysql_fetch_assoc($adminler);
$totalRows_adminler = mysql_num_rows($adminler);
?>
...
<table height="520">
  ...
  <td width="60%" rowspan="2"><a href="../index.php">Anasayfa</a> | <a href="yonet.php">Admin Paneli</a> | <a href="moderatoryonet.php">Moderatör Yönetimi</a><br>
        <br>
        <center>
          <b>Moderatör Yönetimi</b>
        </center><br>
      <center><table style="border: 1px solid black;"><tr><td colspan="2"><center><b>Moderatör Ekle</b></center></td></tr><tr><form action="<?php echo $editFormAction; ?>" method="POST" onsubmit="submitonce(this);" name="adminekle">
      <td style="width:30%; vertical-align:middle;"><b>Kullanýcý:</b></td><td style="width:70%;"><input type="text" name="user"></td></tr><tr><td style="width:30%; vertical-align:middle;"><b>Þifre:</b></td><td style="width:70%;"><input type="password" name="pass"><input type="hidden" name="yetki" value="5"></td></tr><tr><td colspan="2"><center><input type="submit" value="ekle!"></center></td>
        <input type="hidden" name="MM_insert" value="adminekle">
      </form></tr></table></center><br><center><b>Moderatörler:</b></center><?php do { ?>
      <?php echo $row_adminler['user']; ?> - <a href="moderatorsil.php?id=<?php echo $row_adminler['id']; ?>">sil</a><br>
    <?php } while ($row_adminler = mysql_fetch_assoc($adminler)); ?> </td>
