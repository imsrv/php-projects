...
$maxRows_eklenen = 20;
$pageNum_eklenen = 0;
if (isset($_GET['pageNum_eklenen'])) {
  $pageNum_eklenen = $_GET['pageNum_eklenen'];
}
$startRow_eklenen = $pageNum_eklenen * $maxRows_eklenen;

mysql_select_db($database_lyrics, $lyrics);
$query_eklenen = "SELECT * FROM eklenenler ORDER BY id DESC";
$query_limit_eklenen = sprintf("%s LIMIT %d, %d", $query_eklenen, $startRow_eklenen, $maxRows_eklenen);
$eklenen = mysql_query($query_limit_eklenen, $lyrics) or die(mysql_error());
$row_eklenen = mysql_fetch_assoc($eklenen);

if (isset($_GET['totalRows_eklenen'])) {
  $totalRows_eklenen = $_GET['totalRows_eklenen'];
} else {
  $all_eklenen = mysql_query($query_eklenen);
  $totalRows_eklenen = mysql_num_rows($all_eklenen);
}
$totalPages_eklenen = ceil($totalRows_eklenen/$maxRows_eklenen)-1;
...
$maxRows_iletisim = 20;
$pageNum_iletisim = 0;
if (isset($_GET['pageNum_iletisim'])) {
  $pageNum_iletisim = $_GET['pageNum_iletisim'];
}
$startRow_iletisim = $pageNum_iletisim * $maxRows_iletisim;

mysql_select_db($database_lyrics, $lyrics);
$query_iletisim = "SELECT * FROM iletisim ORDER BY id DESC";
$query_limit_iletisim = sprintf("%s LIMIT %d, %d", $query_iletisim, $startRow_iletisim, $maxRows_iletisim);
$iletisim = mysql_query($query_limit_iletisim, $lyrics) or die(mysql_error());
$row_iletisim = mysql_fetch_assoc($iletisim);

if (isset($_GET['totalRows_iletisim'])) {
  $totalRows_iletisim = $_GET['totalRows_iletisim'];
} else {
  $all_iletisim = mysql_query($query_iletisim);
  $totalRows_iletisim = mysql_num_rows($all_iletisim);
}
$totalPages_iletisim = ceil($totalRows_iletisim/$maxRows_iletisim)-1;
?>
<html>
<head>
...
<table height="520">
  ...
  <tr>
    ...
    <td width="60%" rowspan="2"><a href="../index.php">Anasayfa</a> | <a href="yonet.php">Admin Paneli</a> | <a href="iletisim.php">Ýletiþim Talepleri</a><br>
        <br>
        <center>
          <b>Ýletiþim Talepleri</b>
        </center><br>
      <center>
        <?php if ($totalRows_iletisim > 0) { // Show if recordset not empty ?>
        <?php do { ?>
        <table style="border: 1px solid black;">
		<tr>
      <td style="width:30%; vertical-align:middle;"><b>IP Adresi:</b></td>
      <td style="width:70%;"><?php echo $row_iletisim['ip']; ?></td>
    </tr>
  <tr>
    <td style="width:30%; vertical-align:middle;"><b>Ýsim:</b></td>
    <td style="width:70%;"><?php echo $row_iletisim['isim']; ?></td>
    </tr>
    <tr>
      <td style="width:30%; vertical-align:middle;"><b>mail adresi:</b></td>
      <td style="width:70%;"><?php echo $row_iletisim['mail']; ?></td>
    </tr>
    <tr>
      <td style="width:30%; vertical-align:middle;"><b>Konu:</b></td>
      <td style="width:70%;"><?php echo $row_iletisim['konu']; ?></td>
    </tr>
    <tr>
      <td style="width:30%; vertical-align:middle;"><b>Mesaj:</b></td>
      <td style="width:70%;"><textarea name="mesaj" rows="10" cols="30"><?php echo $row_iletisim['mesaj']; ?></textarea></td>
    </tr>
    <tr>
      <td colspan="2"><center>
          <input type="button" value="sil" onClick="window.location=('iletisimsil.php?id=<?php echo $row_iletisim['id']; ?>');">
      </center></td>
  </tr></table>
        <?php } while ($row_iletisim = mysql_fetch_assoc($iletisim)); ?>
        <?php } // Show if recordset not empty ?>
        <?php if ($totalRows_iletisim == 0) { // Show if recordset empty ?>
Ýletiþim talebi yok.
