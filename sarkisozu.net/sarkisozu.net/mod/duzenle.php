...
$maxRows_sarkilistesi = 30;
$pageNum_sarkilistesi = 0;
if (isset($_GET['pageNum_sarkilistesi'])) {
  $pageNum_sarkilistesi = $_GET['pageNum_sarkilistesi'];
}
$startRow_sarkilistesi = $pageNum_sarkilistesi * $maxRows_sarkilistesi;

mysql_select_db($database_lyrics, $lyrics);
$query_sarkilistesi = "SELECT * FROM sarki ORDER BY sanatci ASC, sarki ASC";
$query_limit_sarkilistesi = sprintf("%s LIMIT %d, %d", $query_sarkilistesi, $startRow_sarkilistesi, $maxRows_sarkilistesi);
$sarkilistesi = mysql_query($query_limit_sarkilistesi, $lyrics) or die(mysql_error());
$row_sarkilistesi = mysql_fetch_assoc($sarkilistesi);

if (isset($_GET['totalRows_sarkilistesi'])) {
  $totalRows_sarkilistesi = $_GET['totalRows_sarkilistesi'];
} else {
  $all_sarkilistesi = mysql_query($query_sarkilistesi);
  $totalRows_sarkilistesi = mysql_num_rows($all_sarkilistesi);
}
$totalPages_sarkilistesi = ceil($totalRows_sarkilistesi/$maxRows_sarkilistesi)-1;

$queryString_sarkilistesi = "";
if (!empty($_SERVER['QUERY_STRING'])) {
  $params = explode("&", $_SERVER['QUERY_STRING']);
  $newParams = array();
  foreach ($params as $param) {
    if (stristr($param, "pageNum_sarkilistesi") == false && 
        stristr($param, "totalRows_sarkilistesi") == false) {
      array_push($newParams, $param);
    }
  }
  if (count($newParams) != 0) {
    $queryString_sarkilistesi = "&" . htmlentities(implode("&", $newParams));
  }
}
$queryString_sarkilistesi = sprintf("&totalRows_sarkilistesi=%d%s", $totalRows_sarkilistesi, $queryString_sarkilistesi);
?>
<html>
<head>
...
<table height="520">
  ...
  <tr>
    ...
    <td width="60%" rowspan="2"><a href="../index.php">Anasayfa</a> | <a href="yonet.php">moderatör Paneli</a> | <a href="duzenle.php">Söz Düzenle</a><br>
        <br>
        <center>
          <b>Söz Düzenle:</b>
        </center><br>
        <?php if ($totalRows_sarkilistesi > 0) { // Show if recordset not empty ?>
        <?php do { ?>
        <a href="duzenle_2.php?id=<?php echo $row_sarkilistesi['id']; ?>"><?php echo $row_sarkilistesi['sanatci']; ?> - <?php echo $row_sarkilistesi['sarki']; ?></a><br>
        <?php } while ($row_sarkilistesi = mysql_fetch_assoc($sarkilistesi)); ?>
        <?php } // Show if recordset not empty ?>
        <?php if ($totalRows_sarkilistesi == 0) { // Show if recordset empty ?>
        <center>
        Düzenlenecek þarký sözü bulunamadý.
        </center>
        <?php } // Show if recordset empty ?><br><center>
          <?php if ($pageNum_sarkilistesi > 0) { // Show if not first page ?>
          <a href="<?php printf("%s?pageNum_sarkilistesi=%d%s", $currentPage, 0, $queryString_sarkilistesi); ?>"><<-Ýlk Sayfa</a>
          <?php } // Show if not first page ?>
&nbsp;
<?php if ($pageNum_sarkilistesi > 0) { // Show if not first page ?>
<a href="<?php printf("%s?pageNum_sarkilistesi=%d%s", $currentPage, max(0, $pageNum_sarkilistesi - 1), $queryString_sarkilistesi); ?>"><-Önceki Sayfa</a>
<?php } // Show if not first page ?>
&nbsp;
<?php if ($pageNum_sarkilistesi < $totalPages_sarkilistesi) { // Show if not last page ?>
<a href="<?php printf("%s?pageNum_sarkilistesi=%d%s", $currentPage, min($totalPages_sarkilistesi, $pageNum_sarkilistesi + 1), $queryString_sarkilistesi); ?>">Sonraki Sayfa-></a>
<?php } // Show if not last page ?>
&nbsp;
<?php if ($pageNum_sarkilistesi < $totalPages_sarkilistesi) { // Show if not last page ?>
<a href="<?php printf("%s?pageNum_sarkilistesi=%d%s", $currentPage, $totalPages_sarkilistesi, $queryString_sarkilistesi); ?>">Son Sayfa->></a>
<?php } // Show if not last page ?>
        </center></td>
...
mysql_free_result($sarkilistesi);
?>
