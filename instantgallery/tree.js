...
// Create the tree
function createTree(rootDir, arrName, startNode, openNode) {
	nodes = arrName;
	if (nodes.length > 0) {
		preloadIcons();
		...
		} else document.write("<img src=\"img/base.gif\" align=\"absbottom\" alt=\"\" />Root Gallery Directory -- http://" + rootDir + "/<br />");
...
function addNode(parentNode, recursedNodes) {
	for (var i = 0; i < nodes.length; i++) {
		...
		if (nodeValues[1] == parentNode) {
			...
			// Start link
			if (nodeValues[4] != 0) {
				document.write("<a href=\"#\" onClick=\"postURL('" + nodeValues[3] + "');\" onmouseover=\"window.status='" + nodeValues[2] + "';return true;\" onmouseout=\"window.status=' ';return true;\">");
			}
			...
			} else document.write("<img id=\"icon" + nodeValues[0] + "\" src=\"img/folder.gif\" align=\"absbottom\" alt=\"Page\" />");
			
			// Write out node name
			document.write(nodeValues[2]);
			
			if (nodeValues[4] != 0) { 
			 	document.write("</a>&nbsp;&nbsp;&nbsp;<span class=\"galleryCnt\">(" + nodeValues[4] + " image gallery)</span>");
			}				
