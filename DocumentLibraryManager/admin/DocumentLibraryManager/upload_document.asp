...
If (CStr(UploadFormRequest("MM_update")) = "form1" And CStr(UploadFormRequest("MM_recordId")) <> "") Then
  ...
  MM_fieldsStr  = "ItemLink|value"
  MM_columnsStr = "ItemLink|',none,''"
...
%>
<%
Dim upload__MMcolParam
upload__MMcolParam = "1000"
If (Request.QueryString("ItemID")  <> "") Then 
  upload__MMcolParam = Request.QueryString("ItemID") 
End If
%>
<%
set upload = Server.CreateObject("ADODB.Recordset")
upload.ActiveConnection = MM_documentlibrarymanager_STRING
upload.Source = "SELECT *  FROM DocumentLibrary  WHERE ItemID = " + Replace(upload__MMcolParam, "'", "''") + ""
upload.CursorType = 0
upload.CursorLocation = 2
upload.LockType = 3
upload.Open()
upload_numRows = 0
%>
...
<form ACTION="<%=MM_editAction%>" METHOD="post" enctype="multipart/form-data" name="form1" onSubmit="checkFileUpload(this,'',true,'','','','','','','');return document.MM_returnValue">
  <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableborder">
    <tr valign="baseline"> 
      ...
      <td width="96%"> Update Docucument<strong>: <%=(upload.Fields.Item("ItemName").Value)%></strong></td>
    </tr>
    <tr valign="baseline"> 
      <td nowrap align="right" height="28">&nbsp;</td>
      <td height="28"> <input name="ItemLink" type="file" id="ItemLink" onChange="checkOneFileUpload(this,'',true,'','','','','','','')">
  ...
  <input type="hidden" name="MM_recordId" value="<%= upload.Fields.Item("ItemID").Value %>">
</form>
</body>
</html>
<%
