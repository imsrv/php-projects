...
if (session_is_registered("whossession")) {
    if (($_SESSION['who']) == "admin" || $_SESSION['who'] == "moderator" || $_SESSION['who'] == "user") {
        if (!empty($_POST['title'])) {
            ...
            $subject = $_POST['title'];
			$Subject.="reply to post" . htmlspecialchars($row['subject']);
			...
			$result1 = db_query("SELECT subject FROM {$tableprefix}tblsubforum WHERE subforumid=" . intval($sfid));
            ?>
<html>
<head>
...
<FORM action=msgreply.php method=post name=y
            ...
            <TBODY>
              <TR class=c>
                <TD><TABLE cellPadding=1 cellSpacing=0 width="100%">
                    <TBODY>
                      <TR>
                        <TD noWrap><SPAN class=c>Reply to Post</SPAN></TD>
                ...
                <TD><TABLE cellSpacing=8>
                    <TBODY>
                      <TR>
						...
						<TD width=320><?php while ($row = db_fetch_row($result1)) {
						echo"<INPUT type='text' readonly name='title' size='70' value='reply to - " . $_GET['author'] . "'>";
			}
