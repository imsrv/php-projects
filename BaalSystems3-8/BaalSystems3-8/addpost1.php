...
if (($posting == n) && ($dopost == 'OK')) { // No Free Posting
    if (!empty($_POST['title'])) {
        ...
        $result = db_query("insert into {$tableprefix}tblsubforum(forumid,subject,groupname,authorname,detail,dateposted) values('" . intval($forumid) . "','" . $subject . "','" . $groupname . "','" . $author . "','" . $detail . "',SYSDATE())");
        header("location:subtopic1.php?fid=" . $forumid);
...
<FORM action=addpost1.php method=post name=y
                ...
                <TD><TABLE cellPadding=1 cellSpacing=0 width="100%">
                    <TBODY>
                      <TR>
                        ...
                        <TD align=right noWrap><B><A href="user.php"
style="COLOR: #ffffff">Cancel</A></B></TD>
        ...
        ?>" size=30></TD>
            ...
            echo"<input type='text' size='30' name='groupname' value='" . $row["groupname"] . "'>";
        } 
...
<?php } 
} else if ($posting == y) { // Free Posting
    if (!empty($_POST['title'])) {
        $forumid = $_POST['forumid'];
        $result2 = db_query("select totalpost from {$tableprefix}tblforum where forumid=" . intval($forumid));
        while ($row = db_fetch_row($result2)) {
              ...
              <TR class=a>
                <TD><TABLE cellSpacing=8>
                    <TBODY>
                      ...
                      <TR>
                        <TD align=right noWrap width=90><B>Name <IMG alt="" hspace=2 name=qq src="pics/user1.gif">:</B></TD>
                        <TD width=320><INPUT class=ia maxLength=50 name=name value="<?php echo ($_SESSION['username']);

        ?>" size=30></TD>
                      </TR>
                      <TR>
                        <TD><b>Group&nbsp;Name : <?php print($email); ?></b></TD>
                        <TD><?php while ($row = db_fetch_row($result)) {
            echo"<input type='text' size='30'  name='groupname' value='" . $row["groupname"] . "'>";
        } 
