...
if($HTTP_POST_VARS['todo'] == "proceed") {
    include(DIR_FUNCTIONS."cclib/class.authorize.php");
    ...
    if ($p_gateaway->get_response_code()=="1") {
          $transid_query=bx_db_query("select * from ".$bx_table_prefix."_cctransactions where transid='".$p_gateaway->get_trans_id()."'");
          ...
          else
          {
               ...
               SQL_CHECK(0,"SQL Error at ".__FILE__.":".(__LINE__-1));
               bx_db_insert($bx_table_prefix."_cctransactions","transid,opid,cc_name,cc_type,cc_num,cc_exp,cc_street,cc_city,cc_state,cc_zip,cc_country,cc_phone,cc_email","'".$p_gateaway->get_trans_id()."','".$HTTP_POST_VARS['opid']."','','','','','','','','','','',''");
               bx_session_unregister('curr_opid');
               bx_session_unregister('curr_pmode');
               if(CC_NOTIFY_BUYER == "yes") {
                   $mailfile = $language."/mail/cc_authorizeprocess.txt";
                   ...
                   $invoice_result['auth_address'] = $p_gateaway->get_ret_address();
                   $invoice_result['auth_city'] = $p_gateaway->get_ret_city();
                   $invoice_result['auth_state'] = $p_gateaway->get_ret_state();
                   $invoice_result['auth_zip'] = $p_gateaway->get_ret_zip();
                   $invoice_result['auth_country'] = $p_gateaway->get_ret_country();
                   $invoice_result['auth_phone'] = $p_gateaway->get_ret_phone();
                   $invoice_result['auth_email'] = $p_gateaway->get_ret_email();
                   $invoice_result['order_number'] = $p_gateaway->get_trans_id();
                   $invoice_result['order_status'] = $p_gateaway->get_reason_text();
               ...
               if(CC_NOTIFY_ADMIN == "yes") {
                    ...
                    $themessage.="Authorize.net Order Number: ".$p_gateaway->get_trans_id()."\n";
                    $themessage.="Authorize.net Order Status: ".$p_gateaway->get_reason_text()."\n";
    ...
    else if (($p_gateaway->get_response_code()=="2") || ($p_gateaway->get_response_code()=="3")) {
            $error_message=TEXT_INVALID_CC_PAYMENT;
            if($p_gateaway->get_curl_error()) {
                $text_reason .= "<br>".$p_gateaway->get_curl_error();
            }
            if($p_gateaway->get_reason_text()) {
                $text_reason .= "<br>".$p_gateaway->get_reason_text();
            }
            $back_url=bx_make_url(HTTP_SERVER.FILENAME_MYINVOICES, "auth_sess", $bx_session);
            include(DIR_FORMS.FILENAME_ERROR_FORM);
    }
