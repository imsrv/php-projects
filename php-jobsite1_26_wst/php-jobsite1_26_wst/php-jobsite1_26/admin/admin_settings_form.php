...
if ($HTTP_POST_VARS['todo']=="save") {
      if(ADMIN_SAFE_MODE == "yes") {
            $error_title = "updating Site settings!";
    ...
    <table width="100%" cellspacing="0" cellpadding="2" border="0">
     <tr>
         <td bgcolor="#000000" align="center"><font face="Verdana, Arial" size="2" color="white"><b>Script settings</b></font></td>
     </tr>
     <tr>
       <td bgcolor="#000000">
...
$perms=substr(base_convert(@fileperms(DIR_SERVER_ROOT."application_settings.php"), 10, 8),3);
if ($perms==666 || $perms==777) {
   $perms_error=false;
}
else {
   $title="File Permission Error - application_settings.php";
   $content="<font color=red><b>The permission for this file is invalid (".$perms.").<br>Valid permission for the file is: <b>777</b>.</font><br> The changes to the file will not be saved properly(will be lost)!<br>Please change the file permission for ".DIR_SERVER_ROOT."application_settings.php"." to 777.";
   $perms_error=true;
}
?>
...
$def_lang_select="(select ";
for ($i=0; $i<count($dirs); $i++) {
   if (file_exists(DIR_LANGUAGES.$dirs[$i].".php")) {
           $def_lang_select.=$dirs[$i].",";
   }
}    
$def_lang_select=eregi_replace(",$","",$def_lang_select);
$def_lang_select.=")";
            ...
            echo "<tr>";
                ...
                if ($radio) {
                    ...
                    if (strlen($regexp[2])<30) {
                            ...
                            $regexp[2] = eregi_replace("\\\\'","'",$regexp[2]);
                            $regexp[2] = eregi_replace("\\\\\\\\","\\",$regexp[2]);
                            echo "<a name=\"".$regexp[1]."\"></a><input type=\"text\" name=\"".$regexp[1]."\" value=\"".$regexp[2]."\" size=\"".(strlen($regexp[2])+5)."\">";
                    }
                    else {
                            echo "<td>";
                            $regexp[2] = eregi_replace("\\\\'","&#039",$regexp[2]);
                            $regexp[2] = eregi_replace("\\\\\\\\","\\",$regexp[2]);
                            echo "<a name=\"".$regexp[1]."\"></a><textarea name=\"".$regexp[1]."\" cols=\"50\" rows=\"8\">".eregi_replace('\.&quot;\\\\n&quot;\.',"\n",eregi_replace("'","",$regexp[2]))."</textarea>";
                    }
                }
...
?>
<tr>
        <td colspan="3" align="right"><input type="submit" name="save" value="<?php echo TEXT_BUTTON_SAVE;?>"<?php if($perms_error){ echo " onClick=\"return confirm('Invalid File Permission (".$perms.") for ".eregi_replace("'","\'","application_settings.php")."\\nChanges will not be saved!\\nClick on Ok if you still want to continue, Cancel otherwise!');\"";}?>></td>
</tr>
</table>
</td></tr></table>
</form>
