...
if (!pjs_title) {
    var pjs_title='<b><?php echo bx_js_prepare(TEXT_LATEST_JOBS);?> - <a href="<?php echo bx_js_prepare(HTTP_SERVER);?>"><?php echo bx_js_prepare(SITE_NAME);?></a></b>';
}
if (!pjs_bigFont) {
...
document.write('        </tr>');
<?php
$number_to_display = NR_DISPLAY;
$job_query=bx_db_query("SELECT ".$bx_table_prefix."_jobcategories_".$bx_table_lng.".jobcategory as jobcategory,".$bx_table_prefix."_companies.company, ".$bx_table_prefix."_jobs.jobtitle, ".$bx_table_prefix."_jobs.jobdate, ".$bx_table_prefix."_jobs.compid, ".$bx_table_prefix."_jobs.jobid, concat(if (".$bx_table_prefix."_jobs.city='','',concat(".$bx_table_prefix."_jobs.city, ' - ')),if (".$bx_table_prefix."_jobs.province='','', concat(".$bx_table_prefix."_jobs.province,' - ')), ".$bx_table_prefix."_locations_".$bx_table_lng.".location) as location FROM ".$bx_table_prefix."_jobcategories_".$bx_table_lng.",".$bx_table_prefix."_jobs, ".$bx_table_prefix."_locations_".$bx_table_lng.",".$bx_table_prefix."_companies WHERE TO_DAYS(".$bx_table_prefix."_jobs.jobexpire)>=TO_DAYS(NOW()) and ".$bx_table_prefix."_jobcategories_".$bx_table_lng.".jobcategoryid=".$bx_table_prefix."_jobs.jobcategoryid and ".$bx_table_prefix."_locations_".$bx_table_lng.".locationid=".$bx_table_prefix."_jobs.locationid and (".$bx_table_prefix."_companies.compid=".$bx_table_prefix."_jobs.compid or ".$bx_table_prefix."_jobs.compid=0) GROUP BY ".$bx_table_prefix."_jobs.jobid order by jobdate desc LIMIT 0,".$number_to_display);
