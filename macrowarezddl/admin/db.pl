...
# Create the HTML Output.
    if (%missing_categories) {
        $category_out = qq~<table $tbgrnd $calign $border $cellsp><tr $cellhbg height="10"><td><$font><b>Add</b></font></td>
        ...
        foreach $category (keys %missing_categories) {
            $category_out .= qq|
                <tr><td $bg1><input type=radio name="$category" value="add"></td>
                    <td $bg2><input type=radio name="$category" value="delete"></td>
                    <td $bg1><$font>$category</font></td>
                    <td $bg2><input type=radio name="$category" value="move"></td><td $bg1>| . &build_select_field ("Category", "", "Move-$category") . qq|</tr>|;
            last if $count++ > 10;
        }
