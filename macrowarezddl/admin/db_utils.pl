...
#
    ...
    LINE: while (<DB>) {
        ...
        if ($data[$db_isnew] eq 'Yes') {
            ...
            $output .= qq~
-------------------------------------------------------------------         
$rec{'Program Name'} (added: $rec{'Date'})
$rec{'Program URL'}
$des_q
~;
        }
...
#
    ...
    $output = "<p><table $tbgrnd $calign $border $cellsp width=450>\n";
    foreach $field (@db_cols) {
        next if ($db_form_len{$field} == -1);
        $output .= qq~
            <tr><td align=right valign=top width=20% $bg1><$font>$field:</font></td>
                <td width=80% $bg2><$font>$rec{$field}</font></td></tr>
        ~;
    }
    $output .= "</table></p>\n";
    return $output;
...
#
    ...
    $output = "<p><table $tbgrnd $calign $border $cellsp>";
...
# Set the field name to field-key if we are doing multiple forms.
        ...
        if    ($db_select_fields{"Mult-$field"}) { $output .= "<tr $bg1><td align=right valign=top width=20%><$font>$field:</font></td><td width=80%>" . &build_select_field($field, $rec{$field}, $name, "MULTIPLE SIZE=3") . "</td></tr>\n"; }
        elsif ($db_select_fields{$field})   { $output .= "<tr $bg2><td align=right valign=top width=20%><$font>$field:</font></td><td width=80%>"      . &build_select_field($field, $rec{$field}, $name)    . "</td></tr>\n"; }
        elsif ($db_radio_fields{$field})    { $output .= "<tr $bg1><td align=right valign=top width=20%><$font>$field:</font></td><td width=80%>"      . &build_radio_field($field, $rec{$field}, $name)     . "</td></tr>\n"; }
        elsif ($db_checkbox_fields{$field}) { $output .= "<tr $bg2><td align=right valign=top width=20%><$font>$field:</font></td><td width=80%>"      . &build_checkbox_field ($field, $rec{$field}, $name) . "</td></tr>\n"; }
        elsif ($db_form_len{$field} =~ 
                             /(\d+)x(\d+)/) { $output .= qq~<tr $bg1><td align=right valign=top width=20%><$font>$field:</font></td><td width=80%><textarea wrap="virtual" name="$name" cols="$1" rows="$2">$rec{$field}</textarea></td></tr>\n~; }
        elsif ($db_form_len{$field} == -1)  { $output  = qq~<input type=hidden name="$field" value="$rec{$field}">\n$output~; }
        else                                { $output .= qq~<tr $bg2><td align=right valign=top width=20%><$font>$field:</font></td><td width=80%><input type=text name="$name" value="$rec{$field}" size="$db_form_len{$field}" maxlength="$db_lengths{$field}"></td></tr>\n~; }
    }
    $output .= "</table></p>\n";
    return $output;
}       
