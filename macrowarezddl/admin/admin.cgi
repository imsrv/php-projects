...
# --------------------------------------------------------
    ...
    use CGI;
    require "$db_lib_path/admin.def";
    my $AUTH = CGI->cookie('LinksAUTH');
    if ($AUTH || $in{'login'}) {	
    	my ($U, $P);	
    	if ($in{'login'}) { ($U, $P) = ($in{'Username'}, $in{'Password'}); }	
    	else { ($U, $P) = split /:/, $AUTH; }	
    	open DB, "<$db_script_path/data/admin.db";	
    	LINE: while (<DB> ) {		
    		/^#/    and next LINE;		
    		/^\s*$/ and next LINE;		
    		chomp;		
    		@ADMIN = &split_decode($_);		
    		next unless ($ADMIN[$ADMIN_username] =~ /^$U$/i);		
    		if ($in{'login'}) { ($ADMIN[$ADMIN_password] eq $P) or &html_login ("Invalid Password!"); }		
    		else { (crypt($ADMIN[$ADMIN_password], $P) eq $P) or &html_login ("Session Expired!"); }		
    		$found = 1 and last LINE;	
    	}	
    	close DB;	
    	$found or &html_login ("Invalid Username!");		
if ($in{'login'}) {		
	srand( time() ^ ($$ + ($$ << 15)) );		
	my @salt_chars = ('A' .. 'Z', 0 ..9, 'a' .. 'z', '.', '/');		
	my $salt = join '', @salt_chars[rand 64, rand 64];		
	print CGI->header( -cookie => CGI->cookie ( -name => 'LinksAUTH', -value => ($ADMIN[$ADMIN_username] . ":" . crypt($ADMIN[$ADMIN_password], $salt)), path => '/' ) );		
...
if ($in{'logout'}) {	
	print CGI->header( -cookie => CGI->cookie ( -name => 'LinksAUTH', -value => '', expires => '-1s', path => '/' ) );	
	$html_headers_printed = 1;	
	&html_login ("Logged Out!");
}
($in{'db'} eq "admin" && $ADMIN[$ADMIN_username] ne "NikJazz") and &html_generic ("Unauthorized!", "You are not allowed to access this database!");
