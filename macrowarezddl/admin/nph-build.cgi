...
<body bgcolor=#FFFFFF $link $vlink $alink>
<table width=95% $border $cellsp bgcolor="#000000" $calign>
<tr $cellhbg><td colspan=2>
                    <$font>
                                        <b>Building Links staggered: DirectDownload</b>
                    </font>
            </td></tr>
<tr $bg1><td>
      <H2><$font>
                                  Building Pages - Step: $steps{$step}</font></H2>
                 <PRE><font face="verdana,arial,helvetica" size="1" color="#000000">
...
~;
    CASE: {
        ...
        ($step == 1) and do { 
                ...
                print qq~</PRE><P><B><font face="Verdana" size=1><a href="nph-build.cgi?staggered=1&step=2&auto=$auto">On to Step 2</A></B></FONT></P>~;
                print qq~</font></pre><p></td></tr></table></font></BODY></HTML>~;
                last CASE; 
            };
        ...
        ($step == 2) and do { 
                ...
                ($offset > $#categories) ?
                    print qq~</PRE><P><B><font face="Verdana" size=1><a href="nph-build.cgi?staggered=1&step=3">Go on to Step 3!</A></B></FONT></P>~ :
                    print qq~</PRE><P><B><font face="Verdana" size=1><a href="nph-build.cgi?staggered=1&step=2&limit=$limit&offset=$offset&auto=$auto">Next $limit Categories!</A></B></FONT></P>~;
                    print qq~</font></pre><p></td></tr></table></font></BODY></HTML>~;
                last CASE; 
            };
        ...
        ($step == 3) and do {
                ...
                print qq~</PRE><P><B><font face="Verdana" size=1><a href="nph-build.cgi?staggered=1&step=4&auto=1">Go on to Step 4!</A></B></FONT></P>~;
                                print qq~</font></pre><p></td></tr></table></font></BODY></HTML>~;     
                last CASE;
            };
        ...
        ($step == 4) and do {
                ...
                my $elapsed = time() - $start;
                print "------------------------------------------------\n";
                print qq~Step 4 took $elapsed seconds.\n\n<b><a href="$build_root_url" target="_blank">Your site</a> is now up to date!</b>~;
                print qq~</font></pre><p></td></tr></table></font></BODY></HTML>~;
                last CASE;
            };
...
# Print HTML Header
    $use_html ?
        print qq|<html><head><title>Links Manager: Building Pages</title></head>
<BASE TARGET="_blank">
                 <body bgcolor=#FFFFFF $link $vlink $alink>
<table width=95% $border $cellsp bgcolor="#000000" $calign>
<tr $cellhbg><td colspan=2>
                    <$font>
                                        <b>Building Links: Download</b>
                    </font>
            </td></tr>
<tr $bg1><td>
      <H2><$font>
                                  Building Pages</font></H2>
                 <PRE><font face="verdana,arial,helvetica" size="1" color="#000000">| :
...
# We are finished!  
    ...
    print "</font></pre><p></td></tr></table></font></BODY></HTML>" if ($use_html);
}
...
#
    ...
    LINE: while (<DB>) {
        ...
        $title_linked = &build_linked_title ("$rec{'Category'}/$rec{'Program Name'}");
...
# Creates a Top 10 ratings page.
    ...
    foreach (sort { $b <=> $a } @top_votes) {
        ...
        $top_votes .= qq~<tr><td align=center>$link{'Rating'}</td><td align=center>$link{'Votes'}</td><td><a href="$link{'Program URL'}">$link{'Program Name'}</a></td></tr>\n~;
    }
    foreach (sort { $b <=> $a } @top_rate) {
        $seen{$_}++;
        %link = &array_to_hash ($seen{$_} - 1, @{$top_rate{$_}});
        $top_rated .= qq~<tr><td align=center>$link{'Rating'}</td><td align=center>$link{'Votes'}</td><td><a href="$link{'Program URL'}">$link{'Program Name'}</a></td></tr>\n~;
    }   
...
sub build_top_x {
	...
	for ($i = 0; $i < $display; $i++) {		
		...
		$count = $i+1;		
		$topxhtml .= qq~<tr><td valign="top"><font face=arial size=2><b>$count.</b></font> </td><td valign="top"><font face=arial size=2>~;		
		if ($tmp{'isDetailed'} eq "Yes") { $topxhtml .= qq~<a href="$build_detail_url/$tmp{$db_key}$build_extension" target=_blank><b>$tmp{'Program Name'}</b></a>~; }		
		else { $topxhtml .= qq~<a href="$build_jump_url?$db_key=$tmp{$db_key}" target=_blank><b>$tmp{'Program Name'}</b></a>~; }		
		$topxhtml .= qq~</font><br><font face=arial size=2>($tmp{'Hits'} Downloads)</font></td></tr>\n~;	
	}	
	$topxhtml .= qq~</table>~;	
	return $topxhtml;
}
