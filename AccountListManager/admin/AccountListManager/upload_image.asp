...
If (CStr(UploadFormRequest("MM_update")) <> "" And CStr(UploadFormRequest("MM_recordId")) <> "") Then
  ...
  MM_columnsStr = "AccountImageFile|',none,''"
...
If (Request.QueryString("AccountID")        <> "") Then 
  images__value1 = Request.QueryString("AccountID")       
End If
%>
<%
set images = Server.CreateObject("ADODB.Recordset")
images.ActiveConnection = MM_accountlistmanager_STRING
images.Source = "SELECT *  FROM tblAM_Accounts  WHERE AccountID = " + Replace(images__value1, "'", "''") + ""
images.CursorType = 0
...
<form ACTION="<%=MM_editAction%>" METHOD="post" enctype="multipart/form-data" name="imageA" id="imageA" onSubmit="checkFileUpload(this,'GIF,JPG,JPEG,BMP,PNG',true,'','','','','','','');return document.MM_returnValue">
  <table width="100%" border="0" cellpadding="0" cellspacing="0" class="tableborder">
    <tr valign="baseline"> 
      ...
      <td width="96%"> Upload Image: <strong><%=(images.Fields.Item("AccountName1").Value)%>&nbsp;</strong></td>
    </tr>
    <tr valign="baseline"> 
...
<input type="hidden" name="MM_recordId" value="<%= images.Fields.Item("AccountID").Value %>">
</form>
</body>
</html>
<%
