...
If (Request.QueryString("AccountID")       <> "") Then 
  deletelistitem__MMColParam = Request.QueryString("AccountID")      
End If
%>
<%
set deletelistitem = Server.CreateObject("ADODB.Recordset")
deletelistitem.ActiveConnection = MM_accountlistmanager_STRING
deletelistitem.Source = "SELECT *  FROM tblAM_Accounts  WHERE AccountID = " + Replace(deletelistitem__MMColParam, "'", "''") + ""
...
<input type="hidden" name="MM_recordId" value="<%= deletelistitem.Fields.Item("AccountID").Value %>">
</form>
<%
