...
<?
$query2="SELECT * FROM mcnews_news WHERE valid='Y' ORDER BY date DESC, id DESC LIMIT 0,$nbnews";
...
while($news=mysql_fetch_array($result))
{
         ...
         $query3="SELECT id FROM mcnews_comment WHERE idnews='$news[id]' AND valid='Y'";
         $nbc=mysql_num_rows(mysql_query($query3, $connect));
?>
...
<td bgcolor="<? echo $TDBgcolorTexte; ?>"><font face="<? echo $FontFace; ?>" size="<? echo $FontSizeTexte; ?>" color="<? echo $FontColorTexte; ?>">
         <?
         if ($nbc==0)  {echo $lAucun.' '.$lcomment;}
         elseif ($nbc>1)  {echo $nbc.' '.$lcomments;}
         else         {echo $nbc.' '.$lcomment;}
         if ($nbc>0)
         {
         echo '&nbsp;:&nbsp;&nbsp;';
         echo '<a href="lire.php?n='.$news['id'].'">'.$lLire.'</a>';
         }
         ?>
         &nbsp;&nbsp;&nbsp;
